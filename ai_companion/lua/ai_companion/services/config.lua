...
function Config:GetLocalized(key, ...)
	...
	self.data = {
		...
		Providers = {
			LLM = {
				...
				Defaults = {
					["local"] = {model = "local-model", endpoint = "http://127.0.0.1:1234/v1/chat/completions"},
					...
					openrouter = {model = "meta-llama/llama-3.1-8b-instruct:free", endpoint = "https://openrouter.ai/api/v1/chat/completions"},
					yandexgpt = {model = "yandexgpt-lite", endpoint = "https://llm.api.cloud.yandex.net/foundationModels/v1"},
					gigachat = {model = "GigaChat", endpoint = "https://gigachat.devices.sberbank.ru/api/v1/chat/completions"},
					mistral = {model = "mistral-small-latest", endpoint = "https://api.mistral.ai/v1/chat/completions"},
					together = {model = "meta-llama/Llama-3.2-3B-Instruct-Turbo", endpoint = "https://api.together.xyz/v1/chat/completions"},
				},
			},
			TTS = {
				Default = "elevenlabs",
...
if SERVER then
	concommand.Add("ai_companion_config_debug", function(ply)
		local locator = AICompanion.GetLocator()
		local config = locator and locator:has("config") and locator:get("config")
		local locale = config and config.GetLocalized or function(key) return key end
		if IsValid(ply) and not ply:IsAdmin() then
			print("[AI] " .. locale("settings_admin_only"))
			return
		end
		if config then
			config:DebugPrint()
		else
			print("[AI] " .. locale("error_config_not_found"))
		end
		if IsValid(ply) then
			print("[AI] " .. locale("log_config_debug_console"))
		end
	end)
end
return Config