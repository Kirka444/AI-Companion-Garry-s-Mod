local TTS = {}
-- === ЛОКАЛИЗАЦИЯ через ServiceLocator с кэшированием ===
local L_cache = {}
local function L(key, ...)
	...
	if locator and locator:has("locale") then
		...
		if locale and type(locale.Get) == "function" then
			...
			-- Сохраняем в кэш только если нет дополнительных аргументов
			if select("#", ...) == 0 then
				L_cache[key] = str
			end
			if select("#", ...) > 0 then
...
function TTS:new(utils, config, state)
	...
	if utils and utils.LogDebug then
		utils:LogDebug("[TTS] 🔧 Создание нового экземпляра TTS")  -- <-- ИСПРАВЛЕНО: utils, а не self.utils
	end
	return obj
end
function TTS:init()
...
function TTSProvider:new(config, ttsService)
	local obj = {
		...
		utils = ttsService and ttsService.utils,  -- <-- ссылка на utils
	}
	setmetatable(obj, self)
	return obj
end
...
function TTS:Generate(ply, text, callback)
	...
	-- 1. Базовая проверка валидности игрока
	if not self.utils or not self.utils:IsValid(ply) then
		if callback then callback(nil, "Player invalid") end
		return
	end
	...
	-- 3. Проверка персональной настройки
	...
	-- ВАЖНО: ещё раз проверяем state (дублирование в оригинале)
