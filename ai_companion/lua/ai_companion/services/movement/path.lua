...
function Path:GetLocalized(key, ...)
...
function Path:BuildPath(bot, st, targetPos, moveTarget)
	...
	if self:BuildNoclipPath(bot, st, targetPos, moveTarget) then
		ok = true
	elseif self:BuildDirectPath(bot, st, targetPos) then
		ok = true
	else
		ok = self:BuildNavPath(bot, st, targetPos, moveTarget)
	end

	if ok then
		local const = self:GetConst()

		st.goal_pos = Vector(targetPos)
		st.target_key = self:IsValid(moveTarget) and moveTarget:EntIndex() or -1

		st.repath = st.repath or {}
		st.repath.last_time = CurTime()
		st.repath.cooldown = CurTime() + (const.repathCooldown or 0.8)
	end
...
function Path:ShouldRepath(bot, st, targetPos, moveTarget)
	...
	-- If no path exists, repath immediately (don't wait for cooldown)
	if not st.path or #st.path == 0 or st.path_index > #st.path then
		st.repath.cooldown = CurTime() + 0.05
		return true
	end
	...
	if st.repath.force and CurTime() >= st.repath.next_force then
		...
		if st.target_key ~= key then
			...
			-- Reduced threshold for more responsive following
			local minGoalMove = 32
			local minGoalMoveSq = minGoalMove * minGoalMove

			if goalMoved > minGoalMoveSq then
				-- Smaller cooldown when target is moving
				st.repath.cooldown = CurTime() + 0.3
				return true
			elseif CurTime() - st.repath.last_time > (const.recalcInterval or 1.0)
				   and goalMoved > 64 then
				st.repath.cooldown = CurTime() + 0.5
				return true
			end
		end
	end
