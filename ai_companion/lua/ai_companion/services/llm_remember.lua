-- Dialogue and event memory for LLM context injection.
...
function LLMRemember:AddMessage(playerName, role, text)
	...
	local playerCount = 0
	local botCount = 0
	for i = #pData.messages, 1, -1 do
		local msg = pData.messages[i]
		if msg.role == "player" then
			playerCount = playerCount + 1
			if playerCount > 5 then
				table.remove(pData.messages, i)
			end
		else
			botCount = botCount + 1
			if botCount > BOT_MESSAGE_LIMIT then
				table.remove(pData.messages, i)
			end
		end
	end
...
function LLMRemember:GetDialogHistory(ply)
	if not self:IsEnabled() then return {} end
	if not self.utils or not self.utils:IsValid(ply) then return {} end
	local playerName = self:GetSafePlayerName(ply)
	local pData = self._data.players[playerName]
	if not pData or not pData.messages then return {} end
	local history = {}
	for _, msg in ipairs(pData.messages) do
		local role = msg.role == "player" and "user" or "assistant"
		table.insert(history, { role = role, content = msg.text or "" })
	end
	return history
end

function LLMRemember:ClearPlayerData(playerName)
	playerName = self:SanitizeString(playerName)
	if playerName == "" then return end
	if self._data.players[playerName] then
		self._data.players[playerName] = { messages = {}, events = {} }
		self:MarkDirty()
		if self.utils then
			self.utils.LogInfo("[LLMRemember]",
				self:GetLocalized("log_remember_cleared"),
				playerName)
		end
	end
end
...
function LLMRemember:GetAPI()
	return {
		...
		GetDialogHistory = function(ply) return self:GetDialogHistory(ply) end,
		GetSafePlayerName = function(ply) return self:GetSafePlayerName(ply) end,
		ClearPlayerData = function(playerName) return self:ClearPlayerData(playerName) end,
