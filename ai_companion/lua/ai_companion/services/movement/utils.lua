...
function Utils:LerpAngle(current, target, maxDelta, instant)
	...
	-- Instant snap for non-combat look (no more jerky transitions)
	if instant then
...
function Utils:ApplyWorldMovement(cmd, moveVec, speed)
	...
	-- FIX: Только доминирующее направление, иначе анимация ломается
	local forwardMove = cmd:GetForwardMove()
	local sideMove = cmd:GetSideMove()
	local btns = cmd:GetButtons()
	
	-- Сначала очищаем ВСЕ кнопки направления
	...
	local deadzone = 5  -- Мёртвая зона от дёргания
	
	if absFwd >= absSide then
		-- Доминирует вперёд/назад
