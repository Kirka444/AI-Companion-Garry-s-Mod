...
local function getOrCreateLocator()
	...
	if locator then return locator end
...
function AICompanion.GetLocator()
	...
	if not loc then
		ErrorNoHaltWithStack("[AI Companion] Локатор не найден!\n")
		return nil
	end
	return loc
end
-- 3. РЕГИСТРАЦИЯ ФАБРИК
print("[AI Companion] Регистрация фабрик на", IS_SERVER and "сервере" or "клиенте")

-- Базовые сервисы (нужны всегда)
...
-- Серверные сервисы
if IS_SERVER then
	-- LLM / TTS / Commands (нужны и в соло, и в MP)
	...
	locator:register("commands", function()
		...
		local config = locator:get("config")
		local state = locator:get("state")
		local botmanager = locator:has("botmanager") and locator:get("botmanager") or nil
		local spawn = locator:has("spawn") and locator:get("spawn") or nil
		local llm = locator:get("llm")
		local tts = locator:get("tts")
		local shared = locator:get("shared")
		local vehicle = locator:has("vehicle") and locator:get("vehicle") or nil
	...
	-- === МУЛЬТИПЛЕЕРНЫЕ СЕРВИСЫ (ТОЛЬКО НЕ SOLO) ===
	if not IS_SOLO then
		...
		locator:register("afk", function()
			...
			return AFK:new(utils, config, state, botmanager, spawn, commands, shared, locale)
		end)
	else
		print("[AI Companion] SOLO режим: модули ботов (data, botmanager, spawn,")
		print("  combat, movement, afk) и транспорт (vehicle) пропущены")
	end
end
...
-- 4. АГРЕССИВНАЯ ЗАГРУЗКА СЕРВИСОВ
print("[AI Companion] Агрессивная загрузка сервисов...")

-- Формируем список сервисов для загрузки в зависимости от режима
local allServices = {}

-- Ядро (всегда)
table.insert(allServices, "locale")
table.insert(allServices, "utils")
table.insert(allServices, "state")
table.insert(allServices, "config")
table.insert(allServices, "shared")
table.insert(allServices, "logger")

if IS_SERVER then
	-- LLM/TTS/Commands (всегда на сервере)
	table.insert(allServices, "llm")
	table.insert(allServices, "tts")
	table.insert(allServices, "commands")
	table.insert(allServices, "llm_actions")
	table.insert(allServices, "llm_remember")

	-- Мультиплеерные сервисы (только если НЕ соло)
	if not IS_SOLO then
		table.insert(allServices, "data")
		table.insert(allServices, "botmanager")
		table.insert(allServices, "spawn")
		table.insert(allServices, "vehicle")
		table.insert(allServices, "combat")
		table.insert(allServices, "movement")
		table.insert(allServices, "afk")
	end
end

if IS_CLIENT then
	table.insert(allServices, "botmanager")
	table.insert(allServices, "client")
	table.insert(allServices, "llm_remember")
	table.insert(allServices, "menu")
end
	...
	if locator:has(name) then
		...
		if factory then
			...
			if ok and svc then
				...
				ErrorNoHaltWithStack("[AI Companion] Service load error '" .. name .. "': " .. tostring(svc) .. "\n")
				print("  " .. name .. " — ОШИБКА: " .. tostring(svc))
			end
		else
			print("  " .. name .. " — нет фабрики")
		end
	end
end
...
for _, name in ipairs(allServices) do
	...
	if svc and type(svc.init) == "function" and not svc._initialized then
		...
		if not ok then
			ErrorNoHaltWithStack("[AI Companion] Service init error '" .. name .. "': " .. tostring(err) .. "\n")
			error("[AI Companion] Failed to init '" .. name .. "': " .. tostring(err))
		end
	end
end
-- 6. КЛИЕНТСКИЕ ХУКИ И КОМАНДЫ
if IS_CLIENT then
	...
	concommand.Add("ai_companion_menu", function()
		...
		if loc then
			...
			if menu and menu.Open then menu:Open() end
		end
	end)
end
-- 7. ФИНАЛЬНЫЙ ВЫВОД
function AICompanion.GetLocale()
	local loc = AICompanion.GetLocator()
	if loc and loc:has("locale") then return loc:get("locale") end
