...
function Navigation:GetLocalized(key, ...)
...
function Navigation:GetSmoothedLookAngle(bot, target, maxDelta, instant)
	if not self:IsValid(bot) then return Angle(0, 0, 0) end
	local lookAng = self.utils:GetLookAngle(bot, target)
	local currentAng = bot:EyeAngles()
	return self.utils:LerpAngle(currentAng, lookAng, maxDelta, instant)
end
...
function Navigation:IsNavMeshAvailable()
	...
	if not navmesh or not navmesh.GetNavAreaCount then
		self._navMeshAvailable = false
		return false
	end

	local okGen, generating = pcall(function() return navmesh.IsGenerating() end)
	if not okGen or generating then
		self._navMeshAvailable = false
		return false
	end

	local okCount, count = pcall(function() return navmesh.GetNavAreaCount() end)
	self._navMeshAvailable = okCount and count and count > 0

	if not self._navMeshAvailable and self.utils then
		MsgC(Color(255, 200, 100), "[AI Companion] WARN: " .. self:GetLocalized("log_movement_nav_unavailable") .. "\n")
	end
...
function Navigation:NavigateDirect(bot, cmd, targetPos, moveTarget, st)
	...
	if distToTarget < const.closeFollowDistSq then
		cmd:ClearMovement()
		cmd:ClearButtons()
		local smoothAng = self:GetSmoothedLookAngle(bot, moveTarget or targetPos, 12, true)
...
function Navigation:TryDirectMovement(bot, cmd, targetPos, moveTarget, st)
	...
	local goalMoved = st.direct.goal:DistToSqr(targetPos) > 900

	if now >= st.direct.next or goalMoved then
		local minDirectDist = const.directPathMinDist or 128

		if distToTarget < minDirectDist * minDirectDist then
			st.direct.ok = true
		elseif distToTarget < (const.directPathMaxDistSq or 1000000) then
			...
			st.direct.ok = not trace.Hit and not trace.StartSolid
		else
			st.direct.ok = false
		end

		st.direct.goal = Vector(targetPos)
		st.direct.next = now + (st.direct.ok and 0.25 or 0.6)
	end

	if st.direct.ok then
		self:MoveDirect(bot, cmd, targetPos, moveTarget, st)
		return true
	end
...
function Navigation:MoveDirect(bot, cmd, targetPos, moveTarget, st)
	...
	local jump = self:ShouldJumpCached(bot, dir, blocked, wallTr, nil, st)
...
function Navigation:NavigatePath(bot, cmd, st, moveTarget)
	...
	moveDir:Normalize()
	local now = CurTime()
	local posChanged = st._lastAvoidPos and botPos:DistToSqr(st._lastAvoidPos) > 10000
	if not st.nextAvoid or now >= st.nextAvoid or posChanged then
		st.avoidDir = self:GetAvoidanceDir(bot, moveDir, moveTarget)
		st.nextAvoid = now + 0.3
		st._lastAvoidPos = Vector(botPos)
	end

	moveDir = st.avoidDir or moveDir
	...
	local jump = self:ShouldJumpCached(bot, moveDir, blocked, wallTr, nextWP, st)
	local data = bot._aiBotData or (self.data and self.data:GetBotData(bot))
	...
	local smoothAng = self:GetSmoothedLookAngle(bot, moveTarget or st.goal_pos, 12, true)
	cmd:SetViewAngles(smoothAng)
	bot:SetEyeAngles(smoothAng)
...
function Navigation:ShouldJumpCached(bot, moveDir, blocked, wallTr, nextWP, st)
	local now = CurTime()
	...
	local sig = (blocked and "1" or "0")
		.. ":" .. (nextWP and nextWP.type or "nil")
		.. ":" .. math.floor(moveDir.x * 10)
		.. ":" .. math.floor(moveDir.y * 10)

	if now < st.jump.next and st.jump.sig == sig then
		return st.jump.val
	end

	st.jump.next = now + 0.12
	st.jump.sig = sig
	st.jump.val = self:ShouldJump(bot, moveDir, blocked, wallTr, nextWP)
