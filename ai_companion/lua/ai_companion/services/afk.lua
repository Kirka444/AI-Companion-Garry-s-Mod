local AFK = {}
function AFK:new(utils, config, state, botmanager, spawn, commands, shared, locale)
	local obj = {
		utils = utils,
		config = config,
		state = state,
...
function AFK:_T(key, ...)
	if self.locale and self.locale.Get then
...
function AFK:init()
	...
	if game.SinglePlayer() then
		if self.utils then
			self.utils.LogInfo("AFK", self:_T("log_afk_solo_disabled"))
		end
	...
	self:SetupTimer()
	self:SetupCommands()
	-- Инициализируем всех текущих игроков как активных
	for _, ply in ipairs(player.GetAll()) do
		if self:IsValid(ply) and not ply:IsBot() then
			self:UpdateOwnerActivity(ply)
		end
	end
	self:_SaveState()
	self._initialized = true
	if self.utils then
		self.utils.LogInfo("AFK", self:_T("log_afk_init"), tostring(self._isEnabled))
	end
end
function AFK:_LoadConfig()
	local defaultCfg = {
		InactivityTimeout = 60,
		CheckInterval = 5,  -- Уменьшено с 20 до 5 для быстрой реакции
		ActionInterval = {
			min = 50,
			max = 60,
		},
...
function AFK:_LoadState()
	if self.state then
		...
		if raw and raw.Enabled ~= nil then
			...
			if self.utils then
				self.utils.LogDebug("AFK", "Загружено состояние AFK: " .. tostring(self._isEnabled))
			end
			return
		end
		local enabled = self.state:getSetting("AFK_Enabled")
		if enabled ~= nil then
			...
			if self.utils then
				self.utils.LogDebug("AFK", "Загружено состояние AFK из настроек: " .. tostring(self._isEnabled))
			end
		end
	end
end
...
function AFK:SetEnabled(enabled)
	...
	if SERVER then
		net.Start("gmod.one/ai-companion/afk-sync")
		net.WriteBool(enabled)
		net.Broadcast()
	end
	if self.utils then
		self.utils.LogInfo("AFK",
			self:_T("log_afk_toggle"),
			enabled and self:_T("afk_enabled") or self:_T("afk_disabled"))
	end
end
...
function AFK:UpdateOwnerActivity(owner)
	...
	-- Немедленно проверяем ботов этого владельца
	self:_CheckBotsForOwner(owner)
end
-- Новая функция: немедленная проверка ботов конкретного владельца
function AFK:_CheckBotsForOwner(owner)
	if not self:IsValid(owner) then return end
	local allBots = self:GetAllBots()
	for _, bot in ipairs(allBots) do
		if self:IsValid(bot) and self:SafeAlive(bot) then
			local botOwner = self:GetBotOwner(bot)
			if not self:IsValid(botOwner) then
				botOwner = bot:GetNWEntity("AICompanionOwnerEnt")
			end
			if botOwner == owner then
				self:UpdateAFKForBot(bot)
			end
		end
	end
end
function AFK:IsOwnerActive(owner)
	...
	-- Если lastMove == 0, считаем игрока неактивным (не инициализирован)
	if lastMove == 0 then return false end
...
function AFK:SpawnChairDirect(bot)
	...
	if hullTr.Hit then
		...
		for _, ang in ipairs(angles) do
			...
			local testOffset = Vector(math.cos(rad) * distance,
		...
		if not found then
			for i = 1, 5 do
				...
				for _, ang in ipairs(angles) do
					local rad = math.rad(ang)
					local testOffset = Vector(math.cos(rad) * testDistance,
...
function AFK:ExecuteAFKAction(bot, owner, state, action)
	...
	elseif action == "dance" then
		...
		if chosenAct and type(chosenAct) == "number" then
			xpcall(function() bot:DoAnimationEvent(chosenAct) end,
				function(err)
					if self.utils then
						self.utils.LogDebug("AFK", self:_T("log_afk_anim_error"), tostring(err))
					end
					state.action = "wait"
					return
				end)
	...
	elseif action == "point" then
		...
		if self:IsValid(bot) then
			bot:ChatPrint(self:_T("afk_bot_pointing"))
		end
	else
		state.action = "wait"
		if self:IsValid(bot) then
...
function AFK:UpdateAFKForBot(bot)
	...
	if state.is_afk and not ownerActive then
		...
		elseif state.action == "fight_zombie"
			or state.action == "fight"
			or string.find(state.action or "", "fight_")
		then
...
function AFK:SetupHooks()
	...
	local selfRef = self
	-- Отслеживание движения и действий игрока
	hook.Add("SetupMove", "gmod.one/ai-companion/afk-track-activity", function(ply, _, cmd)
	...
	hook.Add("EntityTakeDamage", "gmod.one/ai-companion/afk-track-damage", function(victim)
		if not selfRef:IsValid(victim) or not victim:IsPlayer() or victim:IsBot() then return end
		selfRef:UpdateOwnerActivity(victim)
	end)
	-- НОВОЕ: Инициализация при первом заходе игрока
	hook.Add("PlayerInitialSpawn", "gmod.one/ai-companion/track-owner-activity", function(ply)
		if not selfRef:IsValid(ply) or ply:IsBot() then return end
		selfRef:UpdateOwnerActivity(ply)
	end)
	-- НОВОЕ: Инициализация при спавне игрока
	hook.Add("PlayerSpawn", "gmod.one/ai-companion/track-revival", function(ply)
		if not selfRef:IsValid(ply) or ply:IsBot() then return end
		selfRef:UpdateOwnerActivity(ply)
	end)
	-- НОВОЕ: Чат тоже считается активностью
	hook.Add("PlayerSay", "gmod.one/ai-companion/track-chat-activity", function(ply)
		if not selfRef:IsValid(ply) or ply:IsBot() then return end
		selfRef:UpdateOwnerActivity(ply)
	end)
	-- НОВОЕ: Смерть/возрождение тоже активность
	hook.Add("PlayerDeath", "gmod.one/ai-companion/track-fatality", function(victim)
		if not selfRef:IsValid(victim) or victim:IsBot() then return end
		selfRef:UpdateOwnerActivity(victim)
	end)
	-- Проверка отключённого состояния для ботов
	hook.Add("SetupMove", "gmod.one/ai-companion/afk-check-disabled", function(ply, _, cmd)
...
function AFK:SetupTimer()
	if not SERVER then return end
	local selfRef = self
	local checkInterval = self._cfg.CheckInterval or 5
	timer.Create("gmod.one/ai-companion/afk-check", checkInterval, 0, function()
		if selfRef.state and selfRef.state:getState("Disabled") then
			return
		end
	...
	if self.utils then
		self.utils.LogInfo("AFK", self:_T("log_afk_timer_start"), checkInterval)
	end
end
function AFK:SetupCommands()
	...
	concommand.Add("ai_companion_afk_toggle", function(ply)
		...
		if not ply:IsAdmin() then
			ply:ChatPrint(selfRef:_T("afk_admin_only"))
			return
		end
		local newState = not selfRef._isEnabled
		selfRef:SetEnabled(newState)
		ply:ChatPrint(selfRef:_T("afk_system_label") ..
				(newState and selfRef:_T("afk_enabled_caps") or selfRef:_T("afk_disabled_caps")))
	end)
	concommand.Add("ai_companion_force_afk", function(ply, cmd, args)
		...
		if not selfRef:IsValid(target) then
			ply:ChatPrint(selfRef:_T("afk_bot_not_found"))
			return
		end
		if selfRef:ForceAFKMode(target, action) then
			ply:ChatPrint(selfRef:_T("afk_mode_enabled_for") ..
					target:Nick() ..
					selfRef:_T("afk_action_label") ..
					action ..
					")")
		else
			ply:ChatPrint(selfRef:_T("afk_mode_failed"))
		end
	end)
	concommand.Add("ai_companion_afk_status", function(ply)
		if not selfRef:IsValid(ply) then return end
		if not ply:IsAdmin() then
			print(selfRef:_T("afk_admin_only"))
			return
		end
		local allBots = selfRef:GetAllBots()
		local afkCount = 0
		...
		print("[AI] " .. selfRef:_T("log_afk_status_title"))
		print("[AI] " ..
			selfRef:_T("log_afk_status_sys",
				selfRef._isEnabled and selfRef:_T("afk_enabled_caps") or selfRef:_T("afk_disabled_caps")))
		print(selfRef:_T("afk_bots_in_afk") .. afkCount .. selfRef:_T("afk_of") .. totalCount)
		print(selfRef:_T("afk_timeout_label") .. selfRef._cfg.InactivityTimeout .. selfRef:_T("afk_seconds"))
	end)
	net.Receive("gmod.one/ai-companion/afk-sync", function(len, ply)
		if not ply or not ply:IsAdmin() then return end
		local enabled = net.ReadBool()
		selfRef:SetEnabled(enabled)
	end)
end
function AFK:GetAPI()
	return {
