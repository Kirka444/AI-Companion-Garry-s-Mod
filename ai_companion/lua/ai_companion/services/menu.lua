...
local GLOBAL_KEYS = {
	...
	LLM_Folder_ID = true, LLM_Client_ID = true, LLM_Scope = true,
...
local function ScaleW(value)
	return math.Clamp(ScrW() * value, 0, ScrW())
end

local function ScaleH(value)
	return math.Clamp(ScrH() * value, 0, ScrH())
end

local function ScaleSize(wFrac, hFrac)
	return ScaleW(wFrac), ScaleH(hFrac)
end
...
function Menu:init()
	...
	if not self.locale then
		self.locale = AICompanion.GetLocale and AICompanion.GetLocale() or {
...
function Menu:RefreshValues()
	if self:IsValid(AICompanionMenuPanel) then
		AICompanionMenuPanel:RefreshValues()
	end
end
...
function Menu:GetAllSettings()
	local now = CurTime()
	-- Увеличиваем TTL кэша до 1 секунды для снижения нагрузки
	if self._settingsCache and self._settingsCacheTime and (now - self._settingsCacheTime) < 1.0 then
	...
	if self.state then
		...
		-- Функция-помощник для безопасного приведения к boolean
		...
		settings.tts_enabled = ToBool(ttsEnabled, false)
		local llmEnabled = self.state:getState("LLM_Enabled")
		settings.llm_enabled = ToBool(llmEnabled, true)
		...
		settings.Locale = self.state:getPlayerSetting(steamID, "Locale") or settings.Locale or "ru"
		if self:IsValid(ply) then
			...
			settings.Prefix_Rainbow    = ToBool(self.state:getPlayerSetting(steamID, "Prefix_Rainbow", false), false)
			settings.Show_Sender_Name  = ToBool(self.state:getPlayerSetting(steamID, "Show_Sender_Name", true), true)
			...
			settings.Stealth_Mode      = ToBool(self.state:getPlayerSetting(steamID, "Stealth_Mode", false), false)
			settings.Defender_Mode     = ToBool(self.state:getPlayerSetting(steamID, "Defender_Mode", false), false)
			settings.Medic_Mode        = ToBool(self.state:getPlayerSetting(steamID, "Medic_Mode", false), false)
			settings.Pacifist_Mode     = ToBool(self.state:getPlayerSetting(steamID, "Pacifist_Mode", false), false)
			settings.Aggressive_Mode   = ToBool(self.state:getPlayerSetting(steamID, "Aggressive_Mode", false), false)
			settings.Custom_Prompt_Enabled = ToBool(self.state:getPlayerSetting(steamID, "Custom_Prompt_Enabled", false), false)
...
function Menu:SendSettingToServer(key, value)
	...
	if self.state then
		local personalKeys = {
			...
			["Locale"] = true,        -- Персональный язык игрока
			["locale"] = true,        -- Совместимость (нижний регистр)
		}
		...
		if isPersonal then
			...
			net.Start("gmod.one/ai-companion/set-setting")
			net.WriteString(key)
			net.WriteString(tostring(value))
			net.SendToServer()
			-- Для языка отправляем также специальный нетворк для мгновенного применения на сервере
			if key == "Locale" or key == "locale" then
...
function Menu:HasCompanion(ply)
	...
	-- Кэширование: проверяем не чаще чем раз в 0.5 сек
	local now = CurTime()
	local cacheKey = tostring(ply:UserID())
	if self._companionCache and self._companionCache[cacheKey] then
		local cache = self._companionCache[cacheKey]
		if now - cache.time < 0.5 then
			return cache.result
		end
	end
	...
	if CLIENT then
		...
		if not result then
			...
			for _, p in ipairs(player.GetAll()) do
				if IsValid(p) then
					self.utils.LogDebug("  ", p:Nick(),
						"IsAICompanion=", p:GetNWBool("IsAICompanion", false),
						"IsBot=", p:IsBot())
				end
			end
		end
	...
	-- Сохраняем в кэш
	if not self._companionCache then self._companionCache = {} end
	self._companionCache[cacheKey] = { result = result, time = now }
	return result
end
...
function Menu:RegisterPanel()
	...
	local PANEL = {}
	PANEL.InputFields = {}
	net.Receive("gmod.one/ai-companion/solo-send-settings", function()
		local json = net.ReadString()
		local data = util.JSONToTable(json)
		-- Используем IsValid() глобальную функцию GMod, а не кастомный метод
		if IsValid(AICompanionMenuPanel) then
			AICompanionMenuPanel._soloSettingsCache = data
			AICompanionMenuPanel:RefreshValues()
		end
	end)
	PANEL.ToggleChecks = {}
	...
	function PANEL:Init()
		...
		self:SetSize(ScaleSize(0.42, 0.56))
		self:Center()
		self:MakePopup()
		...
		if game.SinglePlayer() then
			self:BuildSoloBotTab() -- Ваша новая функция для управления NPC
		else
			self:BuildBotTab()     -- Стандартная вкладка для мультиплеера
			self:BuildCombatTab()  -- Стандартная вкладка боя для мультиплеера
		end
		self:BuildAppearanceTab()
		self:BuildPromptTab()    -- 1. Промт
		self:BuildMemoryTab()    -- 2. Память
		self:BuildWorkflowTab()  -- 3. Workflow
		self:BuildInfoTab()      -- 4. Информация (теперь последняя)

		-- === СОХРАНЕНИЕ ВКЛАДКИ В РАМКАХ СЕССИИ (без файла) ===
		-- Сохраняем активную вкладку при переключении
		local oldOnTabChanged = self.Sheet.OnActiveTabChanged
		self.Sheet.OnActiveTabChanged = function(panel, oldTab, newTab)
			if oldOnTabChanged then
				oldOnTabChanged(panel, oldTab, newTab)
			end
			if IsValid(newTab) and newTab.GetText then
				selfRef._lastActiveTab = newTab:GetText()
			end
		end
		-- Сброс при выходе с сервера / отключении
		if not selfRef._disconnectHookSet then
			selfRef._disconnectHookSet = true
			hook.Add("OnDisconnected", "gmod.one/ai-companion/reset-menu-tab", function()
				selfRef._lastActiveTab = nil
			end)
		end
		-- === КОНЕЦ БЛОКА ===

		-- Восстанавливаем вкладку в следующем кадре, чтобы избежать мигания
		if selfRef._lastActiveTab then
			self:SetVisible(false)
			timer.Simple(0, function()
				if not IsValid(self) or not IsValid(self.Sheet) then return end
				for _, item in ipairs(self.Sheet.Items or {}) do
					if IsValid(item.Tab) and item.Tab:GetText() == selfRef._lastActiveTab then
						self.Sheet:SetActiveTab(item.Tab)
						break
					end
				end
	...
	function PANEL:OnScreenSizeChanged(oldW, oldH)
		self:SetSize(ScaleSize(0.42, 0.56))
		self:Center()
	...
	function PANEL:RefreshAllTabs()
		...
		timer.Simple(0.15, function()
			...
			self._isUpdating = false
			-- Восстанавливаем вкладку после пересоздания
			if selfRef._lastActiveTab then
				for _, item in ipairs(self.Sheet.Items or {}) do
					if IsValid(item.Tab) and item.Tab:GetText() == selfRef._lastActiveTab then
						self.Sheet:SetActiveTab(item.Tab)
	...
	function PANEL:FindSoloBot()
		if not game.SinglePlayer() then return nil end
		for _, ent in ipairs(ents.FindByClass("solo_companion_npc")) do
			if IsValid(ent) then return ent end
		end
		return nil
	end
	function PANEL:RequestSoloSettings()
		self._soloSettingsCache = nil -- Сбрасываем кэш при запросе
		net.Start("gmod.one/ai-companion/solo-request-settings")
		net.SendToServer()
	end
	function PANEL:LoadSoloBotSettings()
		return self._soloSettingsCache
	end
	function PANEL:RefreshValues()
		...
		if game.SinglePlayer() then
			if not self._soloSettingsCache then
				self:RequestSoloSettings()
				self._isUpdating = false
				return
			end
			local settings = self._soloSettingsCache
			if settings then
				if IsValid(self.InputFields["Companion_Nick"]) then
					self.InputFields["Companion_Nick"]:SetText(settings.nick or selfRef:L("companion_default_nick") or "Companion")
				end
				if IsValid(self.InputFields["Model_Path"]) then
					self.InputFields["Model_Path"]:SetText(settings.model or "models/player/urban.mdl")
				end
				if IsValid(self.InputFields["Combat_Weapon"]) then
					self.InputFields["Combat_Weapon"]:SetText(settings.combatWeapon or "weapon_smg1")
				end
				if IsValid(self.InputFields["Idle_Weapon"]) then
					self.InputFields["Idle_Weapon"]:SetText(settings.idleWeapon or "weapon_physgun")
				end

				local function SetToggle(key, val)
					local btn = self.ToggleChecks[key]
					if not IsValid(btn) then return end
					-- Защита от "мгновенного сброса":
					-- если пользователь только что нажал (в пределах 0.5 сек), не обновляем из сервера
					if btn._lastClick and (CurTime() - btn._lastClick) < 0.5 then
						return
					end
					if btn._state ~= val then
						btn._state = val
						btn:UpdateColors()
				...
				local defender = settings.defenderMode
				if defender == nil then defender = true end
				SetToggle("solo_Defender_Mode", defender)

				SetToggle("solo_Stealth_Mode", settings.stealthMode or false)
				SetToggle("solo_Pacifist_Mode", settings.pacifistMode or false)
				SetToggle("solo_Aggressive_Mode", settings.aggressiveMode or false)

				local heal = settings.healEnabled
				if heal == nil then heal = true end
				SetToggle("solo_Heal_Mode", heal)
			end
			self._isUpdating = false
			return
		end
			...
			if selfRef:IsValid(entry) then
				if not entry:IsEditing() then
					...
					if isGlobal and not isAdmin then
						...
						entry:SetTooltip(selfRef:L("global_setting_admin_only"))
					elseif isGlobal and isAdmin then
						entry:SetEditable(true)
						entry:SetTooltip(selfRef:L("global_setting_changes_all"))
					else
						entry:SetEditable(true)
						entry:SetTooltip("")
					end
					...
					if val ~= nil then
						...
						-- Для Custom_Prompt_Text используем SetValue/GetValue вместо SetText/GetText
						-- чтобы сохранить пробелы и обойти ограничение ~512 символов DTextEntry
						if key == "Custom_Prompt_Text" then
							local currentVal = (entry.GetValue and entry:GetValue()) or entry:GetText()
							if currentVal ~= strVal then
								entry._updatingFromServer = true
		...
		for key, btn in pairs(self.ToggleChecks) do
			if selfRef:IsValid(btn) then
				...
				if isGlobal and not isAdmin then
					...
					btn:SetTooltip(selfRef:L("global_setting_admin_only"))
				elseif isGlobal and isAdmin then
					btn:SetEnabled(true)
					btn:SetTooltip(selfRef:L("global_setting_changes_all"))
				else
					btn:SetEnabled(true)
					btn:SetTooltip("")
				end
				-- Защита от перезаписи при активном взаимодействии
				local skipUpdate = btn._lastClick and (CurTime() - btn._lastClick) < 0.5
		...
		if selfRef:IsValid(self.AutoSyncButton) then
			local autoSyncStatus = selfRef:GetSetting("Auto_Sync_Global", true)
					and selfRef:L("mode_on")
					or selfRef:L("mode_off")
	...
	function PANEL:RefreshBotStatus()
		if game.SinglePlayer() then
			if IsValid(self.BotStatusLabel) then
				self.BotStatusLabel:SetText(selfRef:L("solo_mode_title") or "SOLO MODE\n\n" ..
		...
		if selfRef:IsValid(bot) then
			...
			self.BotStatusLabel:SetText(selfRef:L("status_bot_header") .. "\n" ..
				selfRef:L("status_name"):format(bot:Nick() or selfRef:L("unknown")) .. "\n" ..
				selfRef:L("status_health"):format(hp .. "/" .. maxHp)
				.. " | " .. selfRef:L("status_armor"):format(armor) .. "\n" ..
			...
			self.BotStatusLabel:SetText(selfRef:L("status_no_bot_header") .. "\n\n"
	...
	function PANEL:CreateToggle(parent, text, tooltip, default, callback, globalKey, isGlobal)
		...
		lbl:SetWide(ScaleW(0.10))
		lbl:SetTextColor(Color(220, 220, 220))
		if isGlobal then
			lbl:SetTooltip((tooltip or "") .. selfRef:L("global_setting_suffix"))
		else
		...
		btn.DoClick = function(s)
			...
			-- чтобы GetAllSettings() сразу возвращал новое значение
			if selfRef.state then
				if isGlobal then
					selfRef.state:setState(s._key, s._state)
				else
					selfRef.state:setPlayerSetting(ply:SteamID64(), s._key, s._state)
				end
	...
	function PANEL:CreateModeToggle(parent, label, tooltip, options, callback, key, defaultValue, isGlobal)
		...
		btnContainer:SetWide(ScaleW(0.135))
	...
	function PANEL:BuildMainTab()
		...
		langCombo.OnSelect = function(combo, index, value, data)
			...
			if lang and lang ~= selfRef.locale:GetLang() then
				...
				-- SaveToFile() вызывается автоматически на сервере через setPlayerSetting
				if selfRef:IsValid(self.Sheet) then
					self.Sheet:Clear()
				end
				self:BuildMainTab()
				...
				-- Восстанавливаем вкладку после смены языка
				if selfRef._lastActiveTab then
		...
		self:CreateToggle(panel, selfRef:L("menu_tts_personal"), selfRef:L("tooltip_tts_personal"),
			...
			function(val)
				...
				LocalPlayer():ChatPrint("[" .. selfRef:L("ai_prefix") .. "] "
					.. selfRef:L("tts_personal"):format(state))
		...
		self:CreateButton(panel, selfRef:L("menu_reset"), selfRef:L("tooltip_reset"),
			function()
				Derma_Query(selfRef:L("dialog_reset_text"),
		...
		if selfRef:IsValid(ply) and ply:IsAdmin() then
			...
			self:CreateButton(panel, selfRef:L("menu_sync_global"), selfRef:L("tooltip_sync_global"),
				function()
					Derma_Query(selfRef:L("sync_confirm_text"),
						selfRef:L("dialog_replace_title"),
						selfRef:L("dialog_yes"),
						function()
		...
		statusPanel:SetTall(ScaleH(0.13))
		statusPanel.Paint = function(s, w, h)
			...
			draw.SimpleText(selfRef:L("menu_llm_status"):format(llmStatus),
				"DermaDefault", 12, 12, Color(200, 200, 200))
			draw.SimpleText(selfRef:L("menu_tts_status"):format(ttsStatus, personalStatus),
				"DermaDefault", 12, 34, Color(200, 200, 200))
			draw.SimpleText(selfRef:L("menu_debug_status"):format(debugStatus),
				"DermaDefault", 12, 56, Color(200, 200, 200))
			draw.SimpleText(selfRef:L("menu_language") .. ": " .. langStatus,
				"DermaDefault", 12, 78, Color(200, 200, 200))
			local timeoutText = selfRef:L("llm_timeout_label"):format(settings.llm_timeout or 60)
				.. " | " .. selfRef:L("tts_timeout_label"):format(settings.tts_timeout or 120)
			draw.SimpleText(timeoutText, "DermaDefault", 12, 100, Color(180, 180, 200))
		end
		local statusTimer = 0
		statusPanel.Think = function(s)
			...
			if statusTimer > 1.0 then
				statusTimer = 0
				-- InvalidateLayout вызывается только при реальных изменениях данных
				local settings = selfRef:GetAllSettings()
				local llmStatus = ""
				if settings.llm_enabled == false then
					llmStatus = selfRef:L("llm_disabled_status")
				...
				local newHash = tostring(llmStatus) .. tostring(ttsStatus)
					.. tostring(settings.debug_mode)
					.. tostring(selfRef.locale:GetLang())
				if s._lastHash ~= newHash then
					s._lastHash = newHash
					s:InvalidateLayout()
				end
			end
	...
	function PANEL:BuildLLMTab()
		...
		local providerList = {
			...
			{ id = "openrouter", name = "OpenRouter (Free Models)", needsKey = true },
			{ id = "yandexgpt", name = "YandexGPT (Алиса)", needsKey = true, needsFolder = true },
			{ id = "gigachat", name = "GigaChat (Сбер)", needsKey = true, needsClientId = true },
		...
		self:CreateTextEntry(panel, selfRef:L("menu_llm_folder_id") or "Folder ID (YandexGPT)",
			selfRef:L("tooltip_llm_folder_id") or "Folder ID из Яндекс.Облака (только для YandexGPT)",
			selfRef:GetSetting("LLM_Folder_ID", ""),
			function(text)
				selfRef:SendSettingToServer("LLM_Folder_ID", text)
			end, "LLM_Folder_ID", true)
		self:CreateTextEntry(panel, selfRef:L("menu_llm_client_id") or "Client ID (GigaChat)",
			selfRef:L("tooltip_llm_client_id") or "Client ID из СберБизнес API (только для GigaChat)",
			selfRef:GetSetting("LLM_Client_ID", ""),
			function(text)
				selfRef:SendSettingToServer("LLM_Client_ID", text)
			end, "LLM_Client_ID", true)
		self:CreateTextEntry(panel, selfRef:L("menu_llm_scope") or "Scope (GigaChat)",
			selfRef:L("tooltip_llm_scope") or "Область доступа GigaChat (GIGACHAT_API_PERS или GIGACHAT_API_B2B)",
			selfRef:GetSetting("LLM_Scope", "GIGACHAT_API_PERS"),
			function(text)
				selfRef:SendSettingToServer("LLM_Scope", text)
			end, "LLM_Scope", true)
	...
	function PANEL:BuildBotTab()
		...
		self:CreateButton(panel, selfRef:L("menu_create"), selfRef:L("tooltip_create"),
			function()
				...
				if selfRef:HasCompanion(ply) then
					...
					Derma_Query(selfRef:L("dialog_replace_text"):format(name),
						selfRef:L("dialog_replace_title"),
						selfRef:L("dialog_yes"),
						function()
		...
		self:CreateButton(panel, selfRef:L("menu_remove"), selfRef:L("tooltip_remove"),
			function()
				...
				Derma_Query(selfRef:L("dialog_remove_text"),
					selfRef:L("dialog_replace_title"),
					selfRef:L("dialog_yes"),
					function()
						self:RunCommand("ai_companion_remove")
		...
		self:CreateTextEntry(panel, selfRef:L("menu_name"), selfRef:L("tooltip_nick"),
			...
			function(text)
				...
				local bot = selfRef:GetCompanion(ply)
				if selfRef:IsValid(bot) then
					local okName, errName = pcall(function() bot:SetName(text) end)
					if not okName then
						ErrorNoHaltWithStack("[AI Companion Menu] SetName error: " .. tostring(errName) .. "\n")
					end
					local okNick, errNick = pcall(function() bot:SetNick(text) end)
					if not okNick then
						ErrorNoHaltWithStack("[AI Companion Menu] SetNick error: " .. tostring(errNick) .. "\n")
					end
				end
		...
		self:CreateTextEntry(panel, selfRef:L("menu_model"), selfRef:L("tooltip_model"),
			...
			function(text)
				...
				if selfRef:IsValid(ply) then
					...
					local bot = selfRef:GetCompanion(ply)
					if selfRef:IsValid(bot) then
						local okModel, errModel = pcall(function() bot:SetModel(text) end)
						if not okModel then
							ErrorNoHaltWithStack("[AI Companion Menu] SetModel error: " .. tostring(errModel) .. "\n")
						end
					end
				end
	...
	function PANEL:BuildCombatTab()
		...
		self:CreateTextEntry(panel, selfRef:L("menu_combat_weapon"), selfRef:L("tooltip_combat_weapon"),
			...
			function(text)
				...
				-- Через консольную команду — SetBotSetting выдаст оружие автоматически
				ply:ConCommand("ai_companion_combat_weapon " .. text)
				ply:ChatPrint("[" .. selfRef:L("ai_prefix") .. "] " .. selfRef:L("settings_weapon_saved"):format(text))
		...
		self:CreateTextEntry(panel, selfRef:L("menu_melee_weapon"), selfRef:L("tooltip_melee_weapon"),
			...
			function(text)
				...
				ply:ConCommand("ai_companion_melee_weapon " .. text)
				ply:ChatPrint("[" .. selfRef:L("ai_prefix") .. "] " .. selfRef:L("settings_weapon_saved"):format(text))
		...
		self:CreateTextEntry(panel, selfRef:L("menu_idle_weapon"), selfRef:L("tooltip_idle_weapon"),
			...
			function(text)
				...
				ply:ConCommand("ai_companion_idle_weapon " .. text)
				ply:ChatPrint("[" .. selfRef:L("ai_prefix") .. "] " .. selfRef:L("settings_weapon_saved"):format(text))
	...
	function PANEL:BuildAppearanceTab()
		...
		self:CreateToggle(panel, selfRef:L("menu_show_sender_name"), selfRef:L("tooltip_show_sender"),
			appearanceSettings.Show_Sender_Name ~= false,
			function(val)
				local state = val and selfRef:L("mode_on") or selfRef:L("mode_off")
				LocalPlayer():ChatPrint("[" .. selfRef:L("ai_prefix") .. "] "
					.. selfRef:L("menu_show_sender_name") .. ": " .. state)
		...
		examplePanel.Paint = function(s, w, h)
			...
			draw.SimpleText(selfRef:L("menu_example"), "DermaDefault", 8, h/2,
		...
		examplePanel.Think = function(s)
			...
			if showName == nil then showName = true end
			local prefix = settings.Prefix_Text or "[AI]"
			local newHash = tostring(showName) .. tostring(prefix)
			if s._lastHash ~= newHash then
				s._lastHash = newHash
				s:InvalidateLayout()
			end
		...
		self:CreateTextEntry(panel, selfRef:L("menu_red"), selfRef:L("tooltip_color"),
			tostring(appearanceSettings.Prefix_Color_R or 255),
			function(text)
				local ply = LocalPlayer()
				if selfRef:IsValid(ply) then
					local val = tonumber(text)
		...
		self:CreateTextEntry(panel, selfRef:L("menu_green"), selfRef:L("tooltip_color"),
			tostring(appearanceSettings.Prefix_Color_G or 200),
			function(text)
				local ply = LocalPlayer()
				if selfRef:IsValid(ply) then
					local val = tonumber(text)
		...
		self:CreateTextEntry(panel, selfRef:L("menu_blue"), selfRef:L("tooltip_color"),
			tostring(appearanceSettings.Prefix_Color_B or 0),
			function(text)
				local ply = LocalPlayer()
				if selfRef:IsValid(ply) then
					local val = tonumber(text)
		...
		self:CreateToggle(panel, selfRef:L("menu_rainbow_prefix"), selfRef:L("tooltip_rainbow"),
			appearanceSettings.Prefix_Rainbow == true,
			function(val)
				local ply = LocalPlayer()
				if selfRef:IsValid(ply) then
		...
		preview.Think = function(s)
			-- Радужный префикс обновляется каждые 0.1 сек, а не каждый кадр
			s._rainbowTimer = (s._rainbowTimer or 0) + FrameTime()
			if s._rainbowTimer > 0.1 then
				s._rainbowTimer = 0
				local settings = selfRef:GetAllSettings()
				local rainbow = settings.Prefix_Rainbow or false
				if rainbow then
					...
					-- Для не-радужного режима проверяем изменения данных
					local prefix = settings.Prefix_Text or "[AI]"
					local r = settings.Prefix_Color_R or 255
					local g = settings.Prefix_Color_G or 200
					local b = settings.Prefix_Color_B or 0
					local showName = settings.Show_Sender_Name
					if showName == nil then showName = true end
					local newHash = tostring(prefix) .. tostring(r) .. tostring(g) .. tostring(b) .. tostring(showName)
					if s._lastHash ~= newHash then
						s._lastHash = newHash
						s:InvalidateLayout()
					end
	...
	function PANEL:BuildMemoryTab()
		...
		self:CreateToggle(panel, selfRef:L("menu_memory_enabled") or "Включить память",
			...
			function(val)
				...
				net.Start("gmod.one/ai-companion/remember-toggle")
				net.WriteBool(val)
				net.SendToServer()
		...
		info:SetText(selfRef:L("memory_info_text"))
		info:Dock(TOP)
		...
		textContainer:SetTall(ScaleH(0.42))
		...
		memoryLog:SetText(selfRef:L("memory_refresh_hint"))
		...
		if not self._memoryNetSetup then
			...
			net.Receive("gmod.one/ai-companion/remember-send-log", function()
				local logText = net.ReadString()
				if AICompanionMenuPanel and AICompanionMenuPanel.MemoryLog then
					AICompanionMenuPanel.MemoryLog:SetText(logText)
				end
			end)
		end
	...
	function PANEL:BuildInfoTab()
		...
		if not game.SinglePlayer() then
			...
			self.BotStatusLabel:SetTall(ScaleH(0.14))
		...
		about:SetTall(ScaleH(0.32))
		about:SetWrap(true)
		about:SetTextColor(Color(220, 220, 220)) -- Стандартный серый цвет
	...
	self:CreateToggle(panel, selfRef:L("menu_prompt_enabled") or "Use custom prompt",
		...
		function(val)
			...
			local state = val and selfRef:L("mode_on") or selfRef:L("mode_off")
			LocalPlayer():ChatPrint("[" .. selfRef:L("ai_prefix") .. "] "
					.. (selfRef:L("prompt_enabled") or "Custom prompt") .. ": " .. state)
		end, "Custom_Prompt_Enabled", false)

	local sysInfo = vgui.Create("DLabel", panel)
	sysInfo:SetText(selfRef:L("menu_prompt_system_info")
			or "When custom prompt is disabled, the standard system prompt is used.")
	sysInfo:Dock(TOP)
	...
	textContainer:Dock(TOP)
	textContainer:DockMargin(8, 4, 8, 4)
	textContainer:SetTall(ScaleH(0.28))
	textContainer.Paint = function(s, w, h)
	...
	promptEntry:SetAllowNonAsciiCharacters(true)
	...
	promptEntry:SetMaximumCharCount(8000)

	promptEntry.Think = function(s)
		if s:GetMaximumCharCount() ~= 8000 then
			s:SetMaximumCharCount(8000)
		end
	end

	promptEntry:SetText(settings.Custom_Prompt_Text or "")
	promptEntry._lastText = promptEntry:GetText()
	promptEntry._updatingFromServer = false

	promptEntry.SetValue = function(s, text)
		s._updatingFromServer = true
		s:SetText(tostring(text or ""))
		s._lastText = s:GetText()
		timer.Simple(0.3, function()
			if IsValid(s) then s._updatingFromServer = false end
		end)
	end

	promptEntry.GetValue = function(s)
		return s:GetText()
	end

	promptEntry.IsEditing = function(s)
		return s:HasFocus()
	end
	...
	promptEntry.OnValueChange = function(s, text)
		UpdateCharCount()
	end

	promptEntry.OnTextChanged = function(s)
		UpdateCharCount()
	end
	...
	resetBtn:SetWide(ScaleW(0.06))
	resetBtn:SetText(selfRef:L("prompt_reset") or "Reset")
	resetBtn.DoClick = function()
		Derma_Query(selfRef:L("dialog_reset_prompt") or "Reset custom prompt? The standard system prompt will be used.",
			...
			function()
				...
				-- Очищаем на клиенте
				if selfRef.state then
					selfRef.state:setPlayerSetting(steamID, "Custom_Prompt_Text", "")
					selfRef.state:setPlayerSetting(steamID, "Custom_Prompt_Enabled", false)
				end

				-- Отправляем на сервер
				net.Start("gmod.one/ai-companion/set-setting")
				net.WriteString("Custom_Prompt_Text")
				net.WriteString("")
				net.SendToServer()

				net.Start("gmod.one/ai-companion/set-setting")
				net.WriteString("Custom_Prompt_Enabled")
				net.WriteString("false")
				net.SendToServer()

				-- Обновляем тумблер
				local toggleBtn = self.ToggleChecks["Custom_Prompt_Enabled"]
				if IsValid(toggleBtn) then
					toggleBtn._state = false
					toggleBtn:UpdateColors()
					toggleBtn:InvalidateLayout()
				end

				UpdateCharCount()
				LocalPlayer():ChatPrint("[" .. selfRef:L("ai_prefix") .. "] "
					.. (selfRef:L("prompt_reset_done") or "Prompt reset"))
			end,
	...
	if selfRef:IsValid(ply) and ply:IsAdmin() then
		...
		self:CreateToggle(panel, selfRef:L("menu_prompt_allow_custom") or "Allow custom prompts for players",
			...
			function(val)
				...
				local state = val and selfRef:L("mode_on") or selfRef:L("mode_off")
				LocalPlayer():ChatPrint("[" .. selfRef:L("ai_prefix") .. "] "
					.. (selfRef:L("prompt_allow_custom") or "Custom prompts for players")
					.. ": " .. state)
			end, "Allow_Custom_Prompts", true)

		local adminInfo = vgui.Create("DLabel", panel)
		adminInfo:SetText(selfRef:L("menu_prompt_admin_info")
			or "When disabled, regular players cannot change their prompt. Admins can always change it.")
		adminInfo:Dock(TOP)
	...
	local warn = vgui.Create("DLabel", panel)
	warn:SetText(selfRef:L("menu_prompt_warn")
			or "Long prompts may increase response time and token usage. Recommended length: up to 2000 characters.")
	warn:Dock(TOP)
...
function PANEL:BuildWorkflowTab()
		...
		if not selfRef:IsValid(ply) or not ply:IsAdmin() then
			...
			local info = vgui.Create("DLabel", panel)
			info:SetText(selfRef:L("menu_workflow_admin_only")
			or "Workflow management is only available to server admins.")
			info:Dock(TOP)
			info:DockMargin(8, 4, 8, 4)
			info:SetTall(30)
		...
		statusPanel.Paint = function(s, w, h)
			...
			surface.DrawOutlinedRect(0, 0, w, h)
			 ...
			 local filename = settings.TTS_Workflow_Filename
				or settings.tts_workflow_filename
				or (selfRef:L("workflow_none") or "Not loaded")
			local statusText = enabled
				and (selfRef:L("workflow_status_on") or "ON")
				or (selfRef:L("workflow_status_off") or "OFF")
			...
			draw.SimpleText((selfRef:L("workflow_current") or "Current workflow:") .. " " .. filename,
				"DermaDefault",
				12, 12,
				Color(200, 200, 200))
			draw.SimpleText((selfRef:L("workflow_status") or "Status:") .. " " .. statusText,
				"DermaDefault",
				12, 34,
				statusColor)
			draw.SimpleText((selfRef:L("workflow_provider") or "Provider:") .. " ComfyUI (Local TTS)",
				"DermaDefault",
				12, 56,
				Color(0, 191, 255))
		end
		statusPanel.Think = function(s)
			-- Проверяем изменения данных перед перерисовкой
			local settings = selfRef:GetAllSettings()
			local enabled = settings.TTS_Workflow_Enabled or false
			local filename = settings.TTS_Workflow_Filename
				or settings.tts_workflow_filename
				or (selfRef:L("workflow_none") or "Not loaded")
			local newHash = tostring(enabled) .. tostring(filename)
			if s._lastHash ~= newHash then
				s._lastHash = newHash
				s:InvalidateLayout()
			end
		...
		self:CreateButton(panel, selfRef:L("menu_workflow_load") or "Load workflow from file",
			...
			function()
				...
				frame:SetSize(ScaleSize(0.21, 0.14))
				frame:Center()
				frame:MakePopup()
				...
				lbl:SetPos(10, 35)
				lbl:SetWide(ScaleW(0.19))
				lbl:SetTextColor(Color(220, 220, 220))
				local entry = vgui.Create("DTextEntry", frame)
				entry:SetPos(10, 60)
				entry:SetSize(ScaleW(0.19), ScaleH(0.022))
				...
				loadBtn:SetSize(ScaleW(0.09), ScaleH(0.026))
				...
				cancelBtn:SetSize(ScaleW(0.09), ScaleH(0.026))
		...
		self:CreateButton(panel, selfRef:L("menu_workflow_reset") or "Reset to default",
			...
			function()
				Derma_Query(selfRef:L("dialog_reset_workflow") or "Reset workflow to default?",
					selfRef:L("dialog_replace_title") or "Confirm",
					selfRef:L("dialog_yes") or "Yes",
					function()
...
• ai_tts_workflow_status — show status
		...
		infoText:SetTall(ScaleH(0.26))
		infoText:SetWrap(true)
		infoText:SetTextColor(Color(220, 220, 220))
		infoText:SetFont("DermaDefault")
	end
	function PANEL:BuildSoloBotTab()
		local scroll = vgui.Create("DScrollPanel")
		self.Sheet:AddSheet(selfRef:L("menu_bot"), scroll, "icon16/user.png")

		local panel = vgui.Create("DPanel", scroll)
		panel:Dock(TOP)
		panel:SetTall(ScaleH(1.2))
		panel.Paint = function() end
		...
		local settings = self._soloSettingsCache or {}

		-- ============================================================
		-- СЕКЦИЯ: Управление ботом
		-- ============================================================
		self:CreateSection(panel, selfRef:L("menu_solo_control") or "Bot Control")

		self:CreateButton(panel, selfRef:L("menu_solo_create_replace") or "Create / Replace",
			selfRef:L("tooltip_solo_create_replace") or "Spawn or replace companion",
			function()
				LocalPlayer():ConCommand("ai_solo_replace")
				timer.Simple(0.5, function()
					if IsValid(self) then self:RequestSoloSettings() end
				end)
			end)

		self:CreateButton(panel, selfRef:L("menu_solo_remove") or "Remove",
			selfRef:L("tooltip_solo_remove") or "Remove companion from map",
			function()
				LocalPlayer():ConCommand("ai_solo_remove")
			end)

		self:CreateButton(panel, selfRef:L("menu_solo_teleport") or "Teleport",
			selfRef:L("tooltip_solo_teleport") or "Teleport companion to your aim position",
			function()
				LocalPlayer():ConCommand("ai_solo_teleport")
			end)
		...
		local cmdPanel = vgui.Create("DPanel", panel)
		cmdPanel:Dock(TOP)
		cmdPanel:DockMargin(0, 2, 0, 2)
		cmdPanel:SetTall(64)
		cmdPanel.Paint = function() end

		local row1 = vgui.Create("DPanel", cmdPanel)
		row1:Dock(TOP)
		row1:SetTall(30)
		row1.Paint = function() end

		local btnFollow = vgui.Create("DButton", row1)
		btnFollow:Dock(LEFT)
		btnFollow:DockMargin(0, 0, 4, 0)
		btnFollow:SetWide(ScaleW(0.08))
		btnFollow:SetText(selfRef:L("menu_follow"))
		btnFollow:SetTooltip("Say !companion follow")
		btnFollow.DoClick = function()
			LocalPlayer():ConCommand("say !companion follow")
		end

		local btnStop = vgui.Create("DButton", row1)
		btnStop:Dock(LEFT)
		btnStop:DockMargin(0, 0, 4, 0)
		btnStop:SetWide(ScaleW(0.08))
		btnStop:SetText(selfRef:L("menu_stop"))
		btnStop:SetTooltip("Say !companion stop")
		btnStop.DoClick = function()
			LocalPlayer():ConCommand("say !companion stop")
		end

		local btnAttack = vgui.Create("DButton", row1)
		btnAttack:Dock(LEFT)
		btnAttack:DockMargin(0, 0, 4, 0)
		btnAttack:SetWide(ScaleW(0.08))
		btnAttack:SetText(selfRef:L("menu_attack"))
		btnAttack:SetTooltip("Say !companion attack (nearest enemy)")
		btnAttack.DoClick = function()
			LocalPlayer():ConCommand("say !companion attack")
		end

		local btnStatus = vgui.Create("DButton", row1)
		btnStatus:Dock(LEFT)
		btnStatus:DockMargin(0, 0, 0, 0)
		btnStatus:SetWide(ScaleW(0.08))
		btnStatus:SetText(selfRef:L("menu_show_status"))
		btnStatus:SetTooltip("Show companion status")
		btnStatus.DoClick = function()
			LocalPlayer():ConCommand("ai_solo_status")
		end

		local row2 = vgui.Create("DPanel", cmdPanel)
		row2:Dock(TOP)
		row2:DockMargin(0, 4, 0, 0)
		row2:SetTall(30)
		row2.Paint = function() end

		local btnSit = vgui.Create("DButton", row2)
		btnSit:Dock(LEFT)
		btnSit:DockMargin(0, 0, 4, 0)
		btnSit:SetWide(ScaleW(0.08))
		btnSit:SetText(selfRef:L("menu_sit"))
		btnSit:SetTooltip("Enter nearest vehicle")
		btnSit.DoClick = function()
			LocalPlayer():ConCommand("ai_solo_enter_vehicle")
		end

		local btnStandup = vgui.Create("DButton", row2)
		btnStandup:Dock(LEFT)
		btnStandup:DockMargin(0, 0, 0, 0)
		btnStandup:SetWide(ScaleW(0.08))
		btnStandup:SetText(selfRef:L("menu_standup"))
		btnStandup:SetTooltip("Exit current vehicle")
		btnStandup.DoClick = function()
			LocalPlayer():ConCommand("ai_solo_exit_vehicle")
		end

		-- ============================================================
		-- СЕКЦИЯ: Режимы поведения
		-- ============================================================
		self:CreateSection(panel, selfRef:L("menu_behavior"))

		self:CreateToggle(panel,
			selfRef:L("menu_defender"),
			selfRef:L("tooltip_defender"),
			settings.defenderMode ~= false,
			function(val)
				LocalPlayer():ConCommand("ai_solo_defender")
			end, "solo_Defender_Mode", false)

		self:CreateToggle(panel,
			selfRef:L("menu_stealth"),
			selfRef:L("tooltip_stealth"),
			settings.stealthMode or false,
			function(val)
				LocalPlayer():ConCommand("ai_solo_stealth")
			end, "solo_Stealth_Mode", false)

		self:CreateToggle(panel,
			selfRef:L("menu_pacifist"),
			selfRef:L("tooltip_pacifist"),
			settings.pacifistMode or false,
			function(val)
				LocalPlayer():ConCommand("ai_solo_pacifist")
			end, "solo_Pacifist_Mode", false)

		self:CreateToggle(panel,
			selfRef:L("menu_aggressive"),
			selfRef:L("tooltip_aggressive"),
			settings.aggressiveMode or false,
			function(val)
				LocalPlayer():ConCommand("ai_solo_aggressive")
			end, "solo_Aggressive_Mode", false)

		self:CreateToggle(panel,
			selfRef:L("menu_medic"),
			selfRef:L("tooltip_medic"),
			settings.healEnabled ~= false,
			function(val)
				LocalPlayer():ConCommand("ai_solo_heal")
			end, "solo_Heal_Mode", false)

		-- ============================================================
		-- СЕКЦИЯ: Настройки бота
		-- ============================================================
		self:CreateSection(panel, selfRef:L("menu_solo_settings") or "Bot Settings")

		self:CreateTextEntry(panel,
			selfRef:L("menu_solo_nick") or "Name",
			selfRef:L("tooltip_solo_nick") or "Companion display name",
			settings.nick or "Companion",
			function(text)
				if text and text ~= "" then
					LocalPlayer():ConCommand("ai_solo_nick \"" .. text .. "\"")
				end
			end, "Companion_Nick")

		self:CreateTextEntry(panel,
			selfRef:L("menu_solo_model") or "Model",
			selfRef:L("tooltip_solo_model") or "Player model path",
			settings.model or "models/player/urban.mdl",
			function(text)
				if text and text ~= "" then
					LocalPlayer():ConCommand("ai_solo_model \"" .. text .. "\"")
				end
			end, "Model_Path")

		self:CreateTextEntry(panel,
			selfRef:L("menu_solo_combat_weapon") or "Combat Weapon",
			selfRef:L("tooltip_solo_combat_weapon") or "Weapon used in combat (weapon_ar2, weapon_smg1, etc.)",
			settings.combatWeapon or "weapon_smg1",
			function(text)
				if text and text ~= "" then
					LocalPlayer():ConCommand("ai_solo_combat_weapon " .. text)
				end
			end, "Combat_Weapon")

		self:CreateTextEntry(panel,
			selfRef:L("menu_solo_idle_weapon") or "Idle Weapon",
			selfRef:L("tooltip_solo_idle_weapon") or "Weapon used when not in combat",
			settings.idleWeapon or "weapon_physgun",
			function(text)
				if text and text ~= "" then
					LocalPlayer():ConCommand("ai_solo_idle_weapon " .. text)
				end
			end, "Idle_Weapon")
		...
		-- СЕКЦИЯ: Сохранение
		-- ============================================================
		self:CreateSection(panel, selfRef:L("menu_solo_save_section") or "Save / Load")

		local savePanel = vgui.Create("DPanel", panel)
		savePanel:Dock(TOP)
		savePanel:DockMargin(0, 2, 0, 2)
		savePanel:SetTall(32)
		savePanel.Paint = function() end

		local btnSave = vgui.Create("DButton", savePanel)
		btnSave:Dock(LEFT)
		btnSave:DockMargin(0, 0, 8, 0)
		btnSave:SetWide(ScaleW(0.09))
		btnSave:SetText(selfRef:L("menu_solo_save") or "Save Settings")
		btnSave:SetTooltip("Save current settings to file")
		btnSave.DoClick = function()
			LocalPlayer():ConCommand("ai_solo_save")
			chat.AddText(Color(100, 255, 100), "[AI] Settings saved!")
		end

		local btnLoad = vgui.Create("DButton", savePanel)
		btnLoad:Dock(LEFT)
		btnLoad:DockMargin(0, 0, 8, 0)
		btnLoad:SetWide(ScaleW(0.09))
		btnLoad:SetText(selfRef:L("menu_solo_load") or "Load Settings")
		btnLoad:SetTooltip("Load settings from save file")
		btnLoad.DoClick = function()
			LocalPlayer():ConCommand("ai_solo_load")
			timer.Simple(0.3, function()
				if IsValid(self) then self:RequestSoloSettings() end
			end)
		end
...
function Menu:Open()
	if self:IsValid(AICompanionMenuPanel) then
		AICompanionMenuPanel:Remove()
		AICompanionMenuPanel = nil
	end
	AICompanionMenuPanel = vgui.Create("AICompanionMenu")
	AICompanionMenuPanel._menuRef = self
end
function Menu:SetupNetReceivers()
	...
	net.Receive("gmod.one/ai-companion/tts-global-status", function()
		...
		chat.AddText(Color(100, 200, 255), "[AI] TTS "
			.. (status and selfRef:L("mode_on") or selfRef:L("mode_off"))
			.. " " .. selfRef:L("tts_global_status"))
	end)
	-- Обработчики settings-sync / config-sync / player-settings-sync
	-- находятся в shared.lua (единая точка обработки сетевых сообщений)
end
