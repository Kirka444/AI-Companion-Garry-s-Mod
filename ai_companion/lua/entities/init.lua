-- Solo companion NPC initialization and command registration.
...
concommand.Add("ai_solo_combat_weapon", function(ply, cmd, args)
    ...
    if #args < 1 then
        ...
        ply:ChatPrint("[AI] Current combat weapon: " .. tostring(npc._combatWeaponClass))
        return
    end

    local weapon = args[1]

    if npc.SetCombatWeapon and npc:SetCombatWeapon(weapon) then
        ply:ChatPrint("[AI] Combat weapon set: " .. weapon)
        if npc._currentWeaponMode == "combat" then
            npc:SelectWeapon(weapon)
        end
    else
        ply:ChatPrint("[AI] Standard method failed, trying direct...")
        if npc.GiveWeaponDirect then
            local wep = npc:GiveWeaponDirect(weapon)
            if IsValid(wep) then
                npc._combatWeaponClass = weapon
                if npc.SaveSettings then
                    npc:SaveSettings()
                end
                ply:ChatPrint("[AI] Combat weapon set (direct): " .. weapon)
                if npc._currentWeaponMode == "combat" then
                    npc:SelectWeapon(weapon)
                end
            else
                ply:ChatPrint("[AI] Invalid weapon class: " .. weapon)
            end
        else
            ply:ChatPrint("[AI] Invalid weapon class: " .. weapon)
        end
...
concommand.Add("ai_solo_idle_weapon", function(ply, cmd, args)
    ...
    if #args < 1 then
        ...
        ply:ChatPrint("[AI] Current idle weapon: " .. tostring(npc._idleWeaponClass))
        return
    end

    local weapon = args[1]

    if npc.SetIdleWeapon and npc:SetIdleWeapon(weapon) then
        ply:ChatPrint("[AI] Idle weapon set: " .. weapon)
        if npc._currentWeaponMode == "peaceful" then
            npc:SelectWeapon(weapon)
        end
    else
        ply:ChatPrint("[AI] Standard method failed, trying direct...")
        if npc.GiveWeaponDirect then
            local wep = npc:GiveWeaponDirect(weapon)
            if IsValid(wep) then
                npc._idleWeaponClass = weapon
                if npc.SaveSettings then
                    npc:SaveSettings()
                end
                ply:ChatPrint("[AI] Idle weapon set (direct): " .. weapon)
                if npc._currentWeaponMode == "peaceful" then
                    npc:SelectWeapon(weapon)
                end
            else
...
concommand.Add("ai_solo_teleport_safe", function(ply)
    if not IsValid(ply) or not ply:IsPlayer() then return end
    ...
    local candidates = {
        ply:GetPos() + ply:GetForward() * 60 + Vector(0, 0, 5),
        ply:GetPos() + ply:GetRight() * 60 + Vector(0, 0, 5),
        ply:GetPos() - ply:GetRight() * 60 + Vector(0, 0, 5),
        ply:GetPos() - ply:GetForward() * 60 + Vector(0, 0, 5),
        ply:GetPos() + Vector(0, 0, 5),
    }
    ...
    for _, pos in ipairs(candidates) do
        ...
        if not tr.Hit then
            ...
            ply:ChatPrint("[AI] Companion safely teleported!")
            return
        end
    end

    ply:ChatPrint("[AI] Could not find safe teleport position!")
end)
