...
function Client:GetMySetting(key, default)
    ...
    return self.state:getPlayerSetting(ply:SteamID64(), key, default)
end
...
function Client:LoadAvatarConfig()
    ...
    -- Проверяем, существует ли сохранённый файл аватарки
...
function Client:CopyAvatarToMaterials()
    ...
    -- Проверяем, что это картинка (PNG или JPEG)
    ...
    -- Копируем в materials/ai_companion/avatar.png
...
function Client:GetAvatarMat()
    ...
    -- Кастомный путь из команды (приоритет #1)
...
function Client:SetupCommands()
    ...
    -- Загружаем сохранённую аватарку при инициализации
    ...
    concommand.Add("ai_companion_avatar", function(ply, cmd, args)
        ...
        if subcmd == "set" then
            ...
            -- Устанавливаем как кастомный путь
        ...
        elseif subcmd == "reset" or subcmd == "default" then
            ...
            -- Удаляем скопированный файл (опционально)
        ...
        elseif subcmd == "path" and args[2] then
            -- Установка произвольного пути (для продвинутых пользователей)
