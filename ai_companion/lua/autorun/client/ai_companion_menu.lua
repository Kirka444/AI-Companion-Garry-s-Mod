-- Client autorun registration for the AI Companion menu.

if CLIENT then
	local function GetAIMenu()
		if not AICompanion or not AICompanion.GetLocator then return nil end

		local ok, locator = pcall(AICompanion.GetLocator)
	...
	local function GetLocaleString(key, ...)
		if not AICompanion or not AICompanion.GetLocator then return key end
