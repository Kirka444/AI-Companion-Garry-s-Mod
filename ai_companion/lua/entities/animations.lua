...
ENT.Animations = {
	PlayerModel = {
		RunAnimations = {
			["normal"] = ACT_HL2MP_RUN_PISTOL, -- WAS: ACT_HL2MP_RUN (not in urban.mdl!)
		...
		WalkAnimations = {
			["normal"] = ACT_HL2MP_WALK_PISTOL, -- WAS: ACT_HL2MP_WALK (not in urban.mdl!)
		...
		IdleAnimations = {
			["normal"] = ACT_HL2MP_IDLE_PISTOL, -- WAS: ACT_HL2MP_IDLE (not in urban.mdl!)
...
if SERVER then
	function ENT:GetAnimList()
		-- ВСЕГДА используем PlayerModel, так как модель игрока (urban.mdl)
		-- имеет HL2MP анимации. NPCModel содержит старые NPC-анимации
		-- типа walk_all, которые ломают внешний вид.
	...
	function ENT:GetShootAnimation()
		...
		return self:GetAnimList().ShootAnimations[holdtype]
	end
	...
	-- Проигрывает gesture-анимацию выстрела в отдельном слоте,
	-- не заменяя основную анимацию тела (ходьба/бег/простой).
	function ENT:PlayShootGesture()
		if not self:HasWeapon() then return end
		local anim = self:GetShootAnimation()
		if not anim then return end
		-- GESTURE_SLOT_ATTACK = 1 в GMod
		self:PlayAnimation(anim, nil, nil, nil, 1)
	end
	...
	local PASSIVE_HOLD_TYPES = {
		["ar2"] = true,
		["smg"] = true,
		["shotgun"] = true,
	}

	local NORMAL_HOLD_TYPES = {
		["pistol"] = true,
		["revolver"] = true,
		["melee"] = true,
		["fist"] = true,
	...
	function ENT:OnUpdateAnimation()
		...
		if self:IsClimbingDown() then
			return self.ClimbDownAnimation, self.ClimbAnimRate
		end

		-- ВСЕГДА используем реальный holdtype оружия для правильной анимации
		local holdtype = self:HasWeapon() and self:GetWeapon():GetHoldType() or "normal"

		-- ВАЖНО: Gesture-анимации (стрельба) НЕ возвращаем здесь!
		-- Они воспроизводятся отдельно через PlayShootGesture / WeaponPrimaryFire

		if not self:IsOnGround() then
			return anims.JumpAnimations[holdtype], self.JumpAnimRate
		elseif self:IsCrouching() then
			if self:IsMoving() then
				return anims.CrouchWalkAnimations[holdtype], self.CrouchWalkAnimRate
			else
				return anims.CrouchIdleAnimations[holdtype], self.CrouchIdleAnimRate
			end
		elseif self:IsRunning() then
			return anims.RunAnimations[holdtype], self.RunAnimRate
		elseif self:IsMoving() then
			return anims.WalkAnimations[holdtype], self.WalkAnimRate
		else
			return anims.IdleAnimations[holdtype], self.IdleAnimRate
		end
	end
end
