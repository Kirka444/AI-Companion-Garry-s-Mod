-- NavMesh access and pathfinding helpers.
...
function NavMesh:GetAreaID(area)
	if not self:IsValid(area) then return nil end
	local id = area:GetID()
	return id
end
...
function NavMesh:GetAreaAttrCached(area)
	...
	local attrs = tonumber(area:GetAttributes()) or 0
...
function NavMesh:GetAreaClosest(area, pos)
	if not self:IsValid(area) or not pos then return nil end
	local closest = area:GetClosestPointOnArea(pos)
	return closest
end

function NavMesh:GetNearestArea(pos, radius)
	if not navmesh or not navmesh.GetNavAreaCount then return nil end
	if not pos then return nil end
	local area = navmesh.GetNearestNavArea(pos, false, radius or 400)
	return self:IsValid(area) and area or nil
end
...
function NavMesh:GetAreaCorners(area)
	...
	if corners and #corners >= 3 then return corners end
...
function NavMesh:IsAreaStairs(area)
	...
	local adj = area:GetAdjacentAreas()
	if adj and #adj == const.stairsAdjacentThreshold then
...
function NavMesh:GetAreaEntryPoint(toArea, fromArea)
	...
	local corners = self:GetAreaCorners(toArea)
	local bestPoint = nil
	...
	while #open > 0 and iter < const.maxIterations do
		...
		elseif cid == gid then
			...
			elseif area and center then
				local adj2 = area:GetAdjacentAreas()
				if adj2 then
...
function NavMesh:GetNearestAreaSmart(pos, radius)
	...
	for _, zOffset in ipairs(offsets) do
		...
		local area = navmesh.GetNearestNavArea(searchPos, false, radius or const.navAreaSearchRadius)
		if self:IsValid(area) then
			local center = self:GetAreaCenter(area)
			if center then
