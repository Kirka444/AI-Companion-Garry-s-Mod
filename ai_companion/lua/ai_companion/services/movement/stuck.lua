...
function Stuck:GetLocalized(key, ...)
...
function Stuck:Update(bot, cmd, moveDir, st)
	local now = CurTime()
	-- Hard throttle: never update stuck checks more often than every 0.1s
	if st._nextStuckCheck and now < st._nextStuckCheck then
		return false
	end
	st._nextStuckCheck = now + 0.1
