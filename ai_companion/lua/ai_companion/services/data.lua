...
function Data:new(state, config, utils)
	local obj = {
		...
		utils = utils,
		botData = {},
		entIndexToUUID = {},  -- [EntIndex] = UUID для O(1) поиска
		_initialized = false,
		_defaultData = nil,
	}
...
function Data:init()
	...
	-- Создаём шаблон с метатаблицей для быстрого клонирования
...
function Data:SetBotData(bot, data)
	...
	self.entIndexToUUID[entIndex] = uuid  -- O(1) индекс
	if self.utils then
	end
	return true
end
