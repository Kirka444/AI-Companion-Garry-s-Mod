...
function Navigation:NavigatePath(bot, cmd, st, moveTarget)
	...
	moveDir:Normalize()
	local now = CurTime()
	local posChanged = st._lastAvoidPos and botPos:DistToSqr(st._lastAvoidPos) > 10000 -- 100 units
...
function Navigation:UpdateNavigation(bot, cmd, targetPos, moveTarget)
	...
	-- Fallback: try direct movement when NavMesh path is not available
	if self:TryDirectMovement(bot, cmd, targetPos, moveTarget, st) then
		return
	end

	-- Last resort: forced direct navigation
