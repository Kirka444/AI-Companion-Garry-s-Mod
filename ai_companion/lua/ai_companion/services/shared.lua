...
function Shared:SetupNetReceivers()
    ...
    net.Receive("AICompanion_SetGlobalSetting", function(len, ply)
        ...
        if self.state then
            ...
            -- Дублируем в State, иначе сервисы (LLM/TTS/чат-хук) не увидят отключение
            if key == "LLM_Enabled" or key == "TTS_Enabled" then
                self.state:setState(key, value)
                if self.utils then
                    self.utils:LogDebug("[Shared] State." .. key .. " синхронизирован: " .. tostring(value))
                end
            end
...
function Shared:SyncSettingsToClient(ply)
    ...
    if self.state then
        ...
        -- ВАЖНО: нельзя писать `or true/false` — это ломает значение false
...
function Shared:SetupClientReceivers()
    ...
    net.Receive("AI_GlobalSettingUpdated", function()
        ...
        if selfRef.state then
            ...
            -- Включающие флаги дублируем в State (без setState, чтобы не слать эхо на сервер)
            if key == "LLM_Enabled" or key == "TTS_Enabled" then
    ...
    net.Receive("AI_TTS_Global_Status", function()
        ...
        chat.AddText(Color(100, 200, 255), "[AI] TTS " .. (status and "включён" or "выключен") .. " (глобально)")
    end)
...
function Shared:ApplyWeaponHotSwap(bot, botKey, wepClass, botmanager)
    ...
    if not self:SafeGiveWeapon(bot, wepClass) then
        self.utils:LogDebug("[Shared] ⚠️ Не удалось выдать оружие: " .. wepClass)
    end
    ...
    -- Логика выбора совпадает с Commands:SetBotSetting
