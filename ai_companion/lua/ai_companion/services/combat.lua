...
function Combat:init()
	...
	-- Cache frequently used config values as flat locals for fast access
	...
	-- Global grenade cache for zero-GC dodge
	self._grenadeCacheGlobal = {}
	self._grenadeCacheGlobalTime = 0
	-- Cache weapon class strings
...
-- ✅ КЭШИРОВАНИЕ CanSeeTarget НА 0.1 СЕК
...
-- ✅ КЭШИРОВАНИЕ GetTargetAimPos НА 0.2 СЕК
...
-- ✅ ИСПРАВЛЕНО: убрана ошибка с cache
...
-- ✅ КЭШИРОВАНИЕ TraceHull В CombatMovement
...
-- ============================================
-- ✅ ПОЛНОСТЬЮ ОПТИМИЗИРОВАННЫЙ HandleCombat
-- ============================================
function Combat:HandleCombat(bot, cmd, cache)
	...
	-- Fast-path: check vehicle status (cached from StartCommand hook)
	...
	self:InitTargetQueue(botData)
	-- Fast-path: abort check (cached owner)
	...
	-- === CORE COMBAT LOGIC ===
	-- Cache ALL expensive values ONCE per tick
	local now = CurTime()
	local botPos = bot:GetPos()
	local targetPos = target:GetPos()
	-- Use component math for distance (avoid :Distance() C call)
	...
	if doTacticalUpdate then
		...
		-- Melee weapon management (only when close)
			...
			if bot:HasWeapon(meleeWep) then
				...
				if self:IsValid(aw) and aw:GetClass() == meleeWep then
					...
					if aLenSqr > 0.001 then
						...
						-- Lightweight obstacle check (TraceLine instead of TraceHull)
		...
		-- Weapon selection (only during tactical update)
	...
	-- === NON-TACTICAL UPDATE (runs every tick) ===
	-- Use cached values from tactical update
...
function Combat:HandleAimbotCorrection(ent, data)
	...
	-- ✅ ZERO-GC: КОМПОНЕНТНАЯ АРИФМЕТИКА
	local predX = aimPos.x + targetVel.x * timeToTarget
	local predY = aimPos.y + targetVel.y * timeToTarget
	local predZ = aimPos.z + targetVel.z * timeToTarget
...
function Combat:UpdateEnemyCache()
	...
	-- ✅ GetAllBots() ВЫЗЫВАЕТСЯ ОДИН РАЗ, А НЕ В ЦИКЛЕ
...
function Combat:SetupHooks()
	...
	hook.Add("EntityTakeDamage", "gmod.one/ai-companion/assist-on-damage", function(victim, dmg)
		...
		-- === БЫСТРЫЙ ПУТЬ: ищем ботов через ownerIndex ===
		local botsToNotify = {}
		local bm = selfRef.botmanager
		-- Вариант 1: victim — игрок (не бот), ищем его ботов
		...
		-- Вариант 2: attacker — игрок (не бот), ищем его ботов в defender_mode
		...
		for _, bot in ipairs(botsToNotify) do
			...
			-- Owner attacks someone else (assist)
			if IsValid(owner) and attacker == owner and victim ~= bot then
			...
			if IsValid(victim) and victim.IsGlideVehicle then
				...
				if IsValid(driver) and (driver == bot or driver == owner) then
					...
					if attacker == victim then continue end
					if attacker == bot then continue end
					selfRef:RequestTarget(bot, attacker, "protect_vehicle", false)
					local name = selfRef:IsPlayerSafe(attacker) and attacker:Nick() or selfRef:SafeGetClass(attacker)
					SafeChatPrint(owner, selfRef.utils:GetLocalized("bot_protecting_vehicle", name))
				end
				continue
			end
		end
...
-- Обход ai_disabled для AI-Компаньонов
hook.Add("StartCommand", "gmod.one/ai-companion/override-ai-disabled", function(ply, cmd)
	if ply:GetNWBool("IsAICompanion", false) then
		-- Принудительно разрешаем команду, даже при ai_disabled
		-- Логика обрабатывается в Combat/Vehicle хуках
	end
end)
