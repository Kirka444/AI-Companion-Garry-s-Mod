local Vehicle = {}
if SERVER then
	util.AddNetworkString("gmod.one/ai-companion/stop-vehicle-sounds")
end
function Vehicle:new(utils, config, state, data)
	local obj = {
		...
		-- Cached inputs to avoid redundant SetInputFloat/Bool calls
		_lastInputs = {},
	}
...
function Vehicle:GetLocalized(key, ...)
...
function Vehicle:init()
	...
	if not self._hasGlide then
		...
		if self.utils then
			self.utils.LogInfo("Vehicle", self:L("vehicle_log_glide_missing"))
			self.utils.LogInfo("Vehicle", self:L("vehicle_log_glide_install"))
		end
	end
	...
	if self.utils then
		self.utils.LogInfo("Vehicle", self:L("vehicle_log_initialized"),
			self._hasGlide and self:L("yes") or self:L("no"))
	end
end
...
function Vehicle:SanitizeVector(v, fallback, out)
	-- Игнорируем параметр out — всегда возвращаем новый/безопасный вектор
	-- чтобы избежать порчи данных при reuse
	local fallbackVec = fallback or self._zeroVec
	if type(v) ~= "Vector"
		or not self:IsFiniteNumber(v.x)
		or not self:IsFiniteNumber(v.y)
		or not self:IsFiniteNumber(v.z)
	then
		return Vector(fallbackVec)
	end
	return Vector(v)
end
...
function Vehicle:SafeGetVelocity(ent)
	if not self:IsValid(ent) then return self._zeroVec end
	return ent:GetVelocity() or self._zeroVec
end
...
function Vehicle:SafeGetVehicle(ply)
	...
	if not isfunction(ply.GetVehicle) then return nil end
	return ply:GetVehicle()
end
function Vehicle:SafeWorldSpaceCenter(ent)
	if not self:IsValid(ent) then return self._zeroVec end
	return ent:WorldSpaceCenter() or self:SafeGetPos(ent)
end
...
function Vehicle:SafeAlive(ent)
	if not self:IsValid(ent) then return false end
	if not isfunction(ent.Alive) then return false end
	return ent:Alive()
end
function Vehicle:HasGlide()
	if self._hasGlide == nil then
		self._hasGlide = (rawget(_G, "Glide") ~= nil)
	end
...
function Vehicle:ResolveVehicleRoot(ent)
	if not self:IsValid(ent) then
		print("[AI-SIT-ROOT] ResolveVehicleRoot: ent invalid")
		return nil
	end

	local cur = ent
	local startClass = self:SafeGetClass(ent)

	for i = 1, 8 do
		if not self:IsValid(cur) then break end

		-- Если у энтити есть сиденья — это и есть корень Glide-транспорта
		if cur.seats and cur.seats[1] and self:IsValid(cur.seats[1]) then
			return cur
		end

		local parent = self:SafeGetParent(cur)
		if not self:IsValid(parent) then
			return cur
		end
		cur = parent
	end

	return self:IsValid(cur) and cur or ent
end
...
function Vehicle:SetupGlidePatches()
	...
	local selfRef = self
	hook.Add("InitPostEntity", "gmod.one/ai-companion/patch-all-vehicles", function()
		for _, ent in ipairs(ents.GetAll()) do
	...
	hook.Add("OnEntityCreated", "gmod.one/ai-companion/patch-new-vehicle", function(ent)
		if selfRef:IsValid(ent) and selfRef:IsGlideVehicle(ent) then
...
function Vehicle:ApplyVehicleInputs(vehicle, inputs, seatIndex)
	if not self:IsValid(vehicle) then
		if self.utils then
			self.utils.LogDebug("Vehicle", self:L("vehicle_log_invalid_vehicle"))
		end
		return
	end
	if not self:HasGlide() then
		if self.utils then
			self.utils.LogDebug("Vehicle", self:L("vehicle_log_glide_unavailable"))
		end
		return
	end
	if not inputs or type(inputs) ~= "table" then
		if self.utils then
			self.utils.LogDebug("Vehicle", self:L("vehicle_log_invalid_inputs"))
		end
		return
	end
	...
	if vehicle.SetInputFloat then
		...
		if vehicle.SetInputBool then
			vehicle:SetInputBool(seatIndex, "brake", sanitized.brake == true)
			vehicle:SetInputBool(seatIndex, "handbrake", sanitized.handbrake == true)
			if sanitized.attack ~= nil then
				vehicle:SetInputBool(seatIndex, "attack", sanitized.attack == true)
			end
			if sanitized.fire ~= nil then
				vehicle:SetInputBool(seatIndex, "fire", sanitized.fire == true)
			end
		end
end
...
function Vehicle:CalculateHeliInputs(vehicle, target, isEnemy)
	if not self:IsValid(vehicle) or not self:IsValid(target) then
		if self.utils then
			self.utils.LogDebug("Vehicle", self:L("vehicle_log_invalid_heli_params"))
		end
	...
	local approachDist = tonumber(isEnemy and (const.HeliEnemyApproachDist or 200)
		or (const.HeliFollowApproachDist or 350)) or 350
	...
	if currentSpeed > desiredSpeed then
		local dampingFactor = const.HeliDampingFactor or 0.10
		desiredSpeed = desiredSpeed * (1 - dampingFactor) + desiredSpeed * dampingFactor
	end
	...
	local maxRoll = const.HeliMaxRoll or 0.28
	local rollInput = math.Clamp(yawInput * 0.25 + rightErr / 700,
		-maxRoll,
		maxRoll)
...
function Vehicle:UpdateTankAim(bot, root, target, isEnemy)
	...
	if isEnemy and self:IsValid(target) then
		...
		if self:IsValid(target) then
			...
			local fallbackAim = self:SafeGetPos(root)
				+ self:SafeGetForward(root) * 1000
				+ self:SafeGetUp(root) * 50
			aimPos = self:SanitizeVector(fallbackAim, Vector())
		end
	end
	self:SetBotAimTarget(bot, aimPos)
...
function Vehicle:ControlTankCannon(bot, vehicle, target)
	...
	turretOrigin = self:SanitizeVector(root:GetTurretOrigin(), vehPos)
	turretDir = root:GetTurretAimDirection()
	if turretDir then
...
function Vehicle:StopVehicle(root, vehicle, cmd)
	if not self:IsValid(root) then
		if self.utils then
			self.utils.LogDebug("Vehicle", self:L("vehicle_log_invalid_stop_root"))
		end
		return
	end
	...
	if self:IsValid(driverSeat) then
		...
		if not self:IsValid(driver) or not driver:GetNWBool("IsAICompanion", false) then
			if self.utils then
				self.utils.LogDebug("Vehicle", self:L("vehicle_log_driver_not_ai"))
			end
			return
		end
	end
	if self.utils then
		self.utils.LogDebug("Vehicle", self:L("vehicle_log_stopping", self:SafeGetClass(root)))
	end
	...
	if self:HasGlide() and self:IsGlideVehicle(root) then
		...
		if not self._engineStopCooldown[id] or now - self._engineStopCooldown[id] >= 2.0 then
			...
			if root.SetEngineState then root:SetEngineState(0) end
		end
...
function Vehicle:CheckVehicleObstacle(vehicle, direction, distance)
	...
	-- Single TraceHull instead of 3 TraceLines
	local tr = util.TraceHull({
		start = vehPos + Vector(0, 0, 40),
		endpos = vehPos + Vector(0, 0, 40) + forward * distance,
	...
	if not tr.StartSolid and tr.Hit and tr.Fraction < 1 then
		...
		if class ~= "prop_door_rotating" and class ~= "func_door"
		   and not ent.IsPlayer and not ent.IsNPC then
			local hitDist = tr.Fraction * distance
...
function Vehicle:HandleRamDamage(bot, root, vehicle, target, dist)
	...
	if self.utils then
		self.utils.LogDebug("Vehicle", self:L("vehicle_log_ram_hit",
...
function Vehicle:GetAntiStuckState(root, bot)
	...
	local idMissing = not id or id == 0
	if idMissing and self:IsValid(bot) then
		id = bot:EntIndex()
	end
...
function Vehicle:ControlGroundVehicle(bot, root, vehicle, followPos, cmd, ramTarget)
	if not self:IsValid(bot) or not self:IsValid(root) or not followPos then
		if self.utils then
			self.utils.LogDebug("Vehicle", self:L("vehicle_log_invalid_ground_params"))
		end
		return
	end
	local const = self._const or {}
	...
	elseif self:HasGlide() and self:IsGlideVehicle(root) then
		if root.GetEngineState then
				...
				if not engineOn then
					...
					if root.SetEngineState then root:SetEngineState(1) end
				end
		end
	end
	...
	local antiStuckResult = {self:UpdateAntiStuck(bot,
		root,
		vehicle,
		hasObstacle,
		hitNormal,
	...
	if asActive then
		if self.utils then
			self.utils.LogDebug("Vehicle", self:L("vehicle_log_antistuck",
				asThrottle,
				asSteer,
				obstacleDist))
		end
...
function Vehicle:GetDriverSeat(vehicle)
	...
	local root = self:ResolveVehicleRoot(vehicle)
	if not self:IsValid(root) then return nil end

	-- Glide: водительское сиденье всегда seats[1]
	if root.seats and root.seats[1] and self:IsValid(root.seats[1]) then
		return root.seats[1]
	end

	-- Обычный HL2-транспорт
	if self:IsHL2Vehicle(root) then
		return root
	end
...
function Vehicle:FindVehicleEnemy(bot, root, data)
	...
	local botID = bot:EntIndex()
	local now = CurTime()
	local scanInterval = (self._const and self._const.VehicleEnemyScanInterval) or 1.0
	local scanRadius = (self._const and (self._const.RamScanRadius or self._const.VehicleEnemyScanRadius)) or 1500
	...
	local locator = AICompanion.GetLocator()
	local combat = locator and locator:get("combat")
	-- Pre-filter non-target classes
	local nonTargetClasses = {
		["prop_physics"] = true, ["prop_dynamic"] = true,
		["func_door"] = true, ["func_door_rotating"] = true,
		["worldspawn"] = true, ["trigger_multiple"] = true,
	}
	local function consider(ent)
		...
		if nonTargetClasses[class] then return end
	...
	local entsInSphere = ents.FindInSphere(basePos, scanRadius)
	if entsInSphere then
	...
	if self.utils and self:IsValid(bestEnemy) then
		self.utils.LogDebug("Vehicle",
			self:L("vehicle_log_enemy_found",
				bot:Nick(),
...
function Vehicle:ControlVehicle(bot, vehicle, owner, cmd, cache)
	if not self:IsValid(bot) or not self:IsValid(vehicle) then
		if self.utils then
			self.utils.LogDebug("Vehicle", self:L("vehicle_log_invalid_bot_vehicle"))
		end
		return
	end
	if not self:IsValid(owner) then
		if self.utils then
			self.utils.LogDebug("Vehicle", self:L("vehicle_log_invalid_owner"))
		end
		return
	end
	local root = self:GetGlideRoot(vehicle) or vehicle
	if not self:IsValid(root) then
		if self.utils then
			self.utils.LogDebug("Vehicle", self:L("vehicle_log_invalid_root"))
		end
		return
	end
	local data = nil
	...
	-- Use single data reference for all subsequent accesses
	local botData = data
	...
	elseif isDefender then
		local combatTarget = botData and botData.combat and botData.combat.target
		if combatTarget and self:IsValid(combatTarget) and self:SafeAlive(combatTarget) then
			hasEnemy = true
		...
		if not hasEnemy then
			...
			if self:IsValid(foundEnemy) then
				...
				if self.utils then
					self.utils.LogDebug("Vehicle",
						self:L("vehicle_log_combat_target_acquired",
							bot:Nick(),
	...
	if not botIsDriver then
		if self.utils then
			self.utils.LogDebug("Vehicle", self:L("vehicle_log_not_driver"))
		end
		self:StopVehicle(root, vehicle, cmd)
		return
	end
	if self.utils then
		self.utils.LogDebug("Vehicle", self:L("vehicle_log_is_driver",
			tostring(self:GetGlideVehicleType(root)),
			tostring(enemyTarget and self:SafeGetClass(enemyTarget) or self:L("none"))))
	end
	if self:HasGlide() and self:IsGlideVehicle(root) then
...
-- Также обнови EnterBestSeat — замени bot:EnterVehicle на ForceEnterSeat:
...
function Vehicle:EnterBestSeat(bot, vehicle, force, hasEnemy, playerIsDriver, owner)
		...
		if currentVeh ~= bestSeat then
			...
			timer.Simple(0.1, function()
				if self:IsValid(bot) and not bot:InVehicle() then
					self:ForceEnterSeat(bot, bestSeat)
				end
			end)
			return true
		end
	...
	local ok = self:ForceEnterSeat(bot, bestSeat)
...
function Vehicle:FindNearestVehicle(bot, radius)
	...
	for _, ent in ipairs(ents.FindInSphere(botPos, radius)) do
		...
		local root = self:ResolveVehicleRoot(ent)
		if not self:IsValid(root) then continue end

		local rootID = root:EntIndex()
		if seen[rootID] then continue end

		local driverSeat = self:GetDriverSeat(root)
		if not self:IsValid(driverSeat) then continue end

		local driver = self:SafeGetDriver(driverSeat)
		if self:IsValid(driver) then continue end

		seen[rootID] = true
		candidates[#candidates + 1] = {
			root = root,
			dist = botPos:Distance(self:SafeGetPos(root))
		}
	end

	table.sort(candidates, function(a, b) return a.dist < b.dist end)
	return candidates[1] and candidates[1].root or nil
end

function Vehicle:ForceEnterSeat(bot, seat)
	if not self:IsValid(bot) or not self:IsValid(seat) then return false end

	local cur = self:SafeGetDriver(seat)
	if self:IsValid(cur) and cur ~= bot then return false end
	if cur == bot then return true end

	if bot:InVehicle() then
		bot:ExitVehicle()
	end

	local root = self:ResolveVehicleRoot(seat)

	-- Телепорт ровно в сиденье
	bot:SetPos(seat:GetPos())
	local a = seat:GetAngles()
	bot:SetEyeAngles(Angle(a.p, a.y, 0))
	bot:SetCollisionGroup(COLLISION_GROUP_DEBRIS)

	-- Glide: используем SwitchSeat если доступно
	if Glide and Glide.SwitchSeat and seat.GlideSeatIndex and self:IsValid(root) then
		Glide.SwitchSeat(bot, seat.GlideSeatIndex)
	end

	-- Обычный вход
	if not bot:InVehicle() then
		bot:EnterVehicle(seat)
	end

	-- Fallback: назначить водителя напрямую
	if not bot:InVehicle() then
		if seat.SetDriver then seat:SetDriver(bot) end
	end
...
function Vehicle:EnterDriverSeat(bot, vehicle)
	...
	if bot:InVehicle() then
		...
		timer.Simple(0.2, function()
			if self:IsValid(bot) and not bot:InVehicle() then
				self:EnterDriverSeat(bot, vehicle)
			end
		end)
		return true
	end
	...
	-- === HL2 ТРАНСПОРТ (jeep, airboat, и т.д.) ===
	if self:IsHL2Vehicle(root) then
		-- Для HL2 vehicle root = vehicle, садимся напрямую
		bot:SetPos(root:GetPos() + Vector(0, 0, 10))
		bot:SetEyeAngles(root:GetAngles())
		bot:EnterVehicle(root)

		if bot:InVehicle() then
			local data = nil
			if self.botmanager then data = self.botmanager:GetData(bot) end
			if not data and self.data then data = self.data:GetBotData(bot) end
			if data then
				data.vehicle = data.vehicle or {}
				data.vehicle.sit_by_command = true
				data.vehicle.locked_vehicle = root
				data.vehicle.locked_seat = root
				if self.botmanager then self.botmanager:UpdateData(bot, data)
				elseif self.data then self.data:SetBotData(bot, data) end
			end
			return true
		end
		return false
	...
	-- === GLIDE ТРАНСПОРТ ===
	local seat = self:GetDriverSeat(root)
	if not self:IsValid(seat) then
		return false
	end

	local driver = self:SafeGetDriver(seat)
	if self:IsValid(driver) and driver ~= bot then
		return false
	end

	-- Устанавливаем sit_by_command ДО посадки
	local data = nil
	if self.botmanager then data = self.botmanager:GetData(bot) end
	if not data and self.data then data = self.data:GetBotData(bot) end
	if data then
	...
	local ok = self:ForceEnterSeat(bot, seat)

	if ok and bot:InVehicle() then
		local cur = self:SafeGetVehicle(bot)
		if cur ~= seat then
			bot:ExitVehicle()
			timer.Simple(0.2, function()
				if self:IsValid(bot) and not bot:InVehicle() then
					self:ForceEnterSeat(bot, seat)
				end
			end)
			return true
		end

		-- Обновляем данные после успешной посадки
		if data then
			data.vehicle = data.vehicle or {}
			data.vehicle.sit_by_command = true
			data.vehicle.locked_vehicle = root
...
function Vehicle:ForceExit(bot)
	...
	if self:IsValid(currentVeh) then
		...
		if self:HasGlide() and self:IsGlideVehicle(currentRoot) then
			...
			if currentRoot.SetEngineState then currentRoot:SetEngineState(0) end
...
function Vehicle:ResetBotAfterVehicle(bot)
	...
	-- ВОССТАНОВЛЕНИЕ НАСТРОЕК БОТА ИЗ CONFIG
	if data.config then
		-- Восстанавливаем модель
		if data.config.model_path and data.config.model_path ~= "" then
			bot:SetModel(data.config.model_path)
		end
		-- Восстанавливаем оружие (idle/combat)
		local weaponToGive = data.config.idle_weapon or data.config.combat_weapon or "weapon_physgun"
		bot:StripWeapons()
		bot:Give(weaponToGive)
		-- Восстанавливаем цвет (если есть)
		if data.config.player_color then
			bot:SetPlayerColor(data.config.player_color)
		end
		-- Восстанавливаем NW-переменные режимов
		...
		-- Восстанавливаем флаги стелса
		bot._aiStealthCrouch = data.config.stealth_mode or false
		if not data.config.stealth_mode then
	...
	if self.utils then
		self.utils.LogDebug("Vehicle", self:L("vehicle_log_reset_after_vehicle", bot:Nick()))
	end
end
...
function Vehicle:SetupHooks()
	...
	local selfRef = self
	-- ОТКЛЮЧАЕМ РЭГДОЛЛ ДЛЯ AI-КОМПАНЬОНОВ
	hook.Add("Glide_CanRagdollPlayer",
		"gmod.one/ai-companion/glide-can-ragdoll-block",
		function(ply, velocity, unragdollTime)
		if not selfRef:IsValid(ply) then return end
		if ply:GetNWBool("IsAICompanion", false) then
			return false
		end
	end)
		-- Баг 1: Очистка звуков при удалении транспорта
	hook.Add("EntityRemoved", "gmod.one/ai-companion/cleanup-vehicle-sounds", function(ent)
		if not selfRef:IsValid(ent) then return end
		local root = selfRef:GetGlideRoot(ent) or ent
		if not selfRef:IsGlideVehicle(root) then return end
		local bots = {}
		...
		for _, bot in ipairs(bots) do
			if not selfRef:IsValid(bot) or not bot:InVehicle() then continue end
			local botVeh = selfRef:SafeGetVehicle(bot)
			if not selfRef:IsValid(botVeh) then continue end
			local botRoot = selfRef:GetGlideRoot(botVeh) or botVeh
			if botRoot ~= root then continue end
			net.Start("gmod.one/ai-companion/stop-vehicle-sounds")
	...
	hook.Add("PlayerEnteredVehicle",
		"gmod.one/ai-companion/auto-board",
		...
		for _, bot in ipairs(bots) do
			...
			if data.state == "stopped" then continue end

			-- Если бот уже должен сидеть по команде — пропускаем
			if data.vehicle and data.vehicle.sit_by_command then
				continue
			end
			...
			-- === ВОДИТЕЛЬСКОЕ СВОБОДНО — БОТ САДИТСЯ ТУДА (приоритет) ===
			if selfRef:IsValid(driverSeat) and not selfRef:IsValid(selfRef:SafeGetDriver(driverSeat)) then
				local success = selfRef:ForceEnterSeat(bot, driverSeat)
				if success and bot:InVehicle() then
					...
					if selfRef:IsValid(bot) then bot:ChatPrint(selfRef:L("vehicle_enter_driver")) end
					continue
				end
			end

			-- Водительское занято — бот ищет пассажирское
			if selfRef:HasGlide() and selfRef:IsGlideVehicle(root) and root.seats and #root.seats > 1 then
				...
				if selfRef:IsValid(bestSeat) then
					local success = selfRef:ForceEnterSeat(bot, bestSeat)
					if success and bot:InVehicle() then
						if data then
							data.vehicle.locked_vehicle = root
							data.vehicle.locked_seat = bestSeat
						...
						if selfRef:IsValid(bot) then bot:ChatPrint(selfRef:L("vehicle_enter_glide")) end
						continue
					end
				end
			end
			...
			if selfRef:IsValid(hl2Seat) then
				local success = selfRef:ForceEnterSeat(bot, hl2Seat)
				if success and bot:InVehicle() then
					if data then
						data.vehicle.locked_vehicle = root
					...
					if selfRef:IsValid(bot) then bot:ChatPrint(selfRef:L("vehicle_enter_passenger")) end
					continue
				end
			end
		end
	end)
	hook.Add("PlayerLeaveVehicle", "gmod.one/ai-companion/handle-vehicle-exit", function(ply, vehicle)
		if ply:GetNWBool("IsAICompanion", false) then
			selfRef:ResetBotAfterVehicle(ply)
			if selfRef:IsValid(ply) then
				ply:ChatPrint(selfRef:L("vehicle_exit_follow"))
			end
			return
		end
		local bots = {}
	...
	hook.Add("StartCommand", "gmod.one/ai-companion/control-vehicle-command", function(ply, cmd)
		...
		selfRef:ControlVehicle(ply, vehicle, data.owner, cmd, nil)
	end)
	hook.Add("CanExitVehicle", "gmod.one/ai-companion/block-vehicle-exit", function(vehicle, ply)
		if not selfRef:IsValid(ply) then return end
		if not ply:GetNWBool("IsAICompanion", false) then return end
		local data = nil
		if selfRef.botmanager then
		...
		if data and not data.vehicle.sit_by_command then
			if data.owner and data.owner:InVehicle() then
				local ownerVeh = nil
				ownerVeh = data.owner:GetVehicle()
	 ...
	 hook.Add("PlayerDisconnected", "gmod.one/ai-companion/cleanup-abandoned-vehicle", function(ply)
		 if not selfRef:IsValid(ply) then return end
		 if not ply:GetNWBool("IsAICompanion", false) then return end
	 ...
	 hook.Add("PlayerDeath", "gmod.one/ai-companion/cleanup-vehicle-state", function(victim)
