...
function Stealth:GetLocalized(key, ...)
...
-- Восстанавливает стелс-режим из конфига (вызывать после выхода из транспорта/телепорта)
function Stealth:RestoreFromConfig(bot)
	if not self:IsValid(bot) then return false end

	local data = self.data and self.data:GetBotData(bot)
	if not data or not data.config then return false end

	local stealthMode = data.config.stealth_mode or false
	bot:SetNWBool("AI_StealthMode", stealthMode)
	bot._aiStealthCrouch = stealthMode
