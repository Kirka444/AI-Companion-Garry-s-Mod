...
!companion help     - Эта справка]]
    ...
    -- ============================================================
    -- 🔥 ОСТАЛЬНЫЕ КОМАНДЫ — ТРЕБУЮТ БОТА
    -- ============================================================
    local bot = nil
...
-- ============================================================
-- 8. ВСПОМОГАТЕЛЬНЫЕ МЕТОДЫ ДЛЯ КОМАНД
-- ============================================================
...
function LLMActions:IsHostileEntity(ent)
    ...
    -- Проверяем дружественных NPC (через state)
    if self.state and self.state:IsFriendlyNPC(class) then
        return false
    end

    -- Проверяем ботов-компаньонов
...
function LLMActions:SetupChatHook()
    ...
    hook.Add("PlayerSay", "AICompanion_LLMActions", function(ply, text)
        ...
        -- Пропускаем обычные сообщения (их обрабатывает llm.lua)
        return nil
    end)
end
