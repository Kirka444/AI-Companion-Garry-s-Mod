...
if SERVER then
	...
	function ENT:VehicleAttachToSeat()
		...
		if self._vehMode == "hl2" then
			...
			if IsValid(self._vehSeat) then
				pos = self._vehSeat:GetPos() + Vector(0, 0, 8)
				ang = self._vehSeat:GetAngles()
			elseif IsValid(root) then
				pos = root:GetPos() + Vector(0, 0, 8)
				ang = root:GetAngles()
			end

			if IsValid(root) and pos then
				local fwd = Vector(SafeForward(root).x, SafeForward(root).y, 0)

				if fwd:LengthSqr() > 0.001 then
					ang = Angle(0, fwd:Angle().y, 0)
				end
			end
	...
	function ENT:VehicleThink()
		...
		if not IsValid(self._vehRoot) or (self._vehMode == "glide" and self._vehSeat and not IsValid(self._vehSeat)) then
			self:VehicleExit()
		...
		for _, npc in ipairs(ents.FindByClass("solo_companion_npc")) do
			if IsValid(npc) then
				if npc._vehRoot == ent or npc._vehSeat == ent then
					-- Сброс parent до того, как движок удалит бота как дочерний энтити
					npc:SetParent(nil)

					npc._vehRoot = nil
					npc._vehSeat = nil
					npc._vehMode = nil
					npc._vehPassenger = nil
					npc._vehEngineOn = false
					...
					if npc._vehWeaponsSaved then
						pcall(function()
							npc:VehicleRestoreWeapons()
						end)
					end
