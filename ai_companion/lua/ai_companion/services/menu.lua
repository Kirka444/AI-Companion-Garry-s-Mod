...
function Menu:GetAllSettings()
	...
	if self.state then
		...
		settings.tts_enabled = self.state:getState("TTS_Enabled") or false
		settings.llm_enabled = self.state:getState("LLM_Enabled") or true
		...
		if self:IsValid(ply) then
			...
			settings.prefix_r       = self.state:getPlayerSetting(steamID, "Prefix_Color_R", 255)
			settings.prefix_g       = self.state:getPlayerSetting(steamID, "Prefix_Color_G", 200)
			settings.prefix_b       = self.state:getPlayerSetting(steamID, "Prefix_Color_B", 0)
...
function Menu:RegisterPanel()
    ...
    function PANEL:BuildMainTab()
		...
		self:CreateToggle(panel, selfRef:L("menu_debug"), selfRef:L("tooltip_debug"),
			...
			function(val)
				...
				local state = val and selfRef:L("mode_on") or selfRef:L("mode_off")
				self.utils:LogDebug("[AI] " .. selfRef:L("settings_debug"):format(state))
				...
				if locator and locator:has("logger") then
					...
					if val then
						...
						self.utils:LogDebug("[AI] Уровень логирования: DEBUG")
					else
						logger:SetLevel("INFO")
						self.utils:LogDebug("[AI] Уровень логирования: INFO")
					end
				end
			end, "Debug_Mode", true)
        ...
        self:CreateButton(panel, selfRef:L("menu_show_models"), selfRef:L("tooltip_show_models"),
            function()
                ...
                self.utils:LogDebug("")
                self.utils:LogDebug("═══════════════════════════════════════════════════════")
                self.utils:LogDebug("        " .. selfRef:L("menu_show_models"))
                self.utils:LogDebug("═══════════════════════════════════════════════════════")
                self.utils:LogDebug("")

                if not models or next(models) == nil then
                    self.utils:LogDebug("  " .. selfRef:L("no_models_found"))
                else
                    local sorted = {}
        ...
        self:CreateButton(panel, selfRef:L("menu_test_connection"), selfRef:L("tooltip_test_llm"),
            function()
                ...
                if selfRef:IsValid(ply) then
                    ...
                    self.utils:LogDebug("[AI MENU] " .. selfRef:L("llm_testing"))
                end
            end)
    end
        ...
        self:CreateButton(panel, selfRef:L("menu_test_tts"), selfRef:L("tooltip_test_tts"),
            function()
                ...
                if selfRef:IsValid(ply) then
                    ...
                    self.utils:LogDebug("[AI MENU] " .. selfRef:L("tts_testing"))
                end
            end)
    end
    ...
    function PANEL:BuildAppearanceTab()
		...
		self:CreateToggle(panel, selfRef:L("menu_show_sender_name"), selfRef:L("tooltip_show_sender"),
			...
			function(val)
				selfRef:SendSettingToServer("Show_Sender_Name", val)
				local state = val and selfRef:L("mode_on") or selfRef:L("mode_off")
...
• ai_tts_workflow_status — show status
    ...
    -- Сохраняем ссылку на PANEL для использования
	...
	net.Receive("AI_PlayerSettings_Sync", function()
		...
		if ok and settings and type(settings) == "table" then
			...
			if selfRef:IsValid(ply) and selfRef.state then
				local steamID = ply:SteamID64()
				-- 🔥 Заполняем кэш персональных настроек
