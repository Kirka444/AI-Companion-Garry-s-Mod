-- Vehicle handling for the solo companion NPC.
...
if SERVER then
	...
	local function Passenger_GetSeatDriver(seat)
		...
		-- Фолбэк для HL2 / prisoner_pod
		if not IsValid(driver) and IsValid(seat) then
			local class = SafeClass(seat)
			if class == "prop_vehicle_prisoner_pod" or class:find("prop_vehicle", 1, true) then
				-- Поиск игрока
				for _, ply in ipairs(player.GetAll()) do
					if IsValid(ply) and ply:InVehicle() then
						if ply:GetVehicle() == seat then
							driver = ply
							break
						end
					end
				...
				if not IsValid(driver) then
					for _, ent in ipairs(ents.GetAll()) do
						if IsValid(ent) and (ent:IsNPC() or ent:IsNextBot()) then
							local entVeh = nil
							pcall(function() entVeh = ent:GetVehicle() end)
							local entParent = ent:GetParent()

							if entVeh == seat or entParent == seat then
								driver = ent
								break
							end
						end
	...
	local function Passenger_FindAttachedFreeSeat(root)
		...
		local masterVeh = IsValid(master) and master:InVehicle() and master:GetVehicle() or nil
		...
		for _, ent in ipairs(ents.GetAll()) do
			if IsValid(ent) and ent ~= root and Passenger_IsSeatClass(ent) then
				...
				if parent == root or dist < PASSENGER_SEAT_SEARCH_RADIUS then
					-- Не садимся в место мастера
					if ent == masterVeh then continue end

					-- Для HL2 подов без родителя: принимаем только ближе 120 юнитов
					local class = SafeClass(ent)
					if class == "prop_vehicle_prisoner_pod" then
						if not IsValid(parent) and ent ~= root then
							if dist > 120 then
								continue
							end
						end
	...
	function ENT:VehicleCanEnter(ent)
		...
		elseif mode == "hl2" then
			local driver = Passenger_GetSeatDriver(seat)

			if IsValid(driver) and driver:IsPlayer() and driver ~= self then
				return false
			end
		end
	...
	function ENT:VehicleAttachToSeat()
		...
		if self._vehMode == "glide" and seat and not IsValid(seat) then
			self:SetParent(nil)
			self:VehicleExit()
		...
		if self._vehMode == "hl2" then
			...
			-- Для HL2 подов: если seat валиден и отличается от root,
			-- привязываемся к seat, а не к root
			local attachTarget = root
			if IsValid(seat) and seat ~= root then
				attachTarget = seat
			end

			if not IsValid(attachTarget) then
				self:SetParent(nil)
				return
			end

			local lPos, lAng = CalculateHL2LocalSeatTransform(attachTarget)

			self:SetParent(attachTarget)
	...
	function ENT:VehicleEnter(ent)
		...
		if mode == "hl2" then
			...
			if IsValid(driver) and driver:IsPlayer() and driver ~= self then
				self:ChatPrint("[AI] Транспорт занят игроком.")
	...
	function ENT:PassengerEnter(root, seat, mode)
		...
		if self:VehicleIsValid() then return true end
		if not IsValid(root) or not IsValid(seat) then return false end

		-- Не садимся в место мастера
		local master = GetMaster()
		local masterVeh = IsValid(master) and master:InVehicle() and master:GetVehicle() or nil
		if seat == masterVeh then
			return false
		end
	...
	local function FindSafeExitPosFromSeat(root, seat)
		if not IsValid(root) and not IsValid(seat) then
			return Vector(0, 0, 0)
		end

		local base = IsValid(seat) and SafePos(seat) or SafePos(root)

		-- Отодвигаем на 15 юнитов в случайном направлении
		local angle = math.random() * math.pi * 2
		local offset = Vector(math.cos(angle) * 15, math.sin(angle) * 15, 0)

		return base + offset + Vector(0, 0, 5)
	end
	...
	function ENT:VehicleExit()
		...
		if not IsValid(root) or (self._vehMode == "glide" and not IsValid(seat)) then
			...
			self:SwitchToPeaceful()
			self._forceIdleNextTick = true

			if self.loco and self.loco.SetDesiredSpeed then
				self.loco:SetDesiredSpeed(0)
			end

			local master2 = GetMaster()
			if IsValid(master2) then
				local tpPos = master2:GetPos() + master2:GetForward() * 60
				local downTr = util.TraceLine({
					start = tpPos + Vector(0, 0, 100),
					endpos = tpPos - Vector(0, 0, 500),
					mask = MASK_SOLID,
				})
				if downTr.Hit then
					self:SetPos(downTr.HitPos + Vector(0, 0, 2))
				else
					self:SetPos(tpPos + Vector(0, 0, 5))
				end
			end
		...
		if self._vehPassenger then
			...
			if self._vehMode == "hl2" and IsValid(seat) and seat ~= root then
				-- Выход из отдельного пода
				exitPos = FindSafeExitPosFromSeat(root, seat)
			else
				-- Стандартная логика для Glide или водительского места
				exitPos = Passenger_FindSafeExitPos(root, seat)

				local downTr = util.TraceLine({
					start = exitPos + Vector(0, 0, 100),
					endpos = exitPos - Vector(0, 0, 500),
					mask = MASK_SOLID,
				})
				if downTr.Hit then
					exitPos = downTr.HitPos + Vector(0, 0, 2)
				end
			end
			...
			-- Возвращаем коллизию через короткий промежуток
			timer.Simple(0.1, function()
				if IsValid(self) then
					self:SetNotSolid(false)
					self:SetCollisionGroup(self._vehOldCollision or COLLISION_GROUP_PLAYER)
		...
		local master = GetMaster()
		if IsValid(master) then
			local basePos = master:GetPos()
			local found = false

			local candidates = {
				basePos + master:GetForward() * 60,
				basePos + master:GetRight() * 60,
				basePos - master:GetRight() * 60,
				basePos - master:GetForward() * 60,
				basePos,
			}
			...
			for _, candidate in ipairs(candidates) do
				...
				if downTr.Hit then
					local groundPos = downTr.HitPos + Vector(0, 0, 2)

					local tr = util.TraceHull({
						start = groundPos + Vector(0, 0, 50),
						endpos = groundPos,
						mins = Vector(-16, -16, 0),
			...
			if not found then
				local downTr = util.TraceLine({
					start = basePos + Vector(0, 0, 100),
					endpos = basePos - Vector(0, 0, 500),
					mask = MASK_SOLID,
				})
				if downTr.Hit then
					self:SetPos(downTr.HitPos + Vector(0, 0, 2))
	...
	function ENT:TryPassengerBoard(master)
		if not IsValid(self) or not self:Alive() then return false end
		if self:VehicleIsValid() then return true end
		if not IsValid(master) or not master:InVehicle() then return false end

		local plyVeh = master:GetVehicle()
		if not IsValid(plyVeh) then return false end

		local plyRoot = Passenger_GetMasterVehicleRoot(master)
		if not IsValid(plyRoot) then return false end

		local plyMode = nil
		local r, m = GetVehicleRoot(plyVeh)
		if IsValid(r) then plyMode = m end
		if not plyMode then
			if IsGlideVehicle(plyRoot) then plyMode = "glide"
			elseif IsHL2Vehicle(plyRoot) then plyMode = "hl2"
			else plyMode = "hl2" end
		end
