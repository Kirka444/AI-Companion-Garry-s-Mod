-- Shared utility helpers used across the addon.
...
function Utils:CleanForTTS(str)
    ...
    str = string.gsub(str, "[\240-\244][\128-\191][\128-\191][\128-\191]", " ")
    ...
    if str == "" then
        return "Отправился пустой текст, пожалуйста, проверьте консоль КомфиЮАЙ на наличие проблем"
    end
...
function Utils:URLEncode(str)
	if not str then return "" end
	return string.gsub(str, "([^%w%-_%.%not %*%'%(%)])", function(c)
