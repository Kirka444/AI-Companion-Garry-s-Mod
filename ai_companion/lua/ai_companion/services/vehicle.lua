-- Vehicle control and driving behavior for companion bots.
...
function Vehicle:ControlVehicle(bot, vehicle, owner, cmd, cache)
	...
	elseif isDefender then
		...
		if hasEnemy and self:IsValid(owner) and enemyTarget == owner then
			...
			if trigger ~= "command"
				and trigger ~= "command_llm"
				and trigger ~= "protect_vehicle"
			then
...
function Vehicle:ResetBotAfterVehicle(bot)
	...
	if data.config then
		if data.config.model_path and data.config.model_path ~= "" then
			if self.utils and self.utils.IsValidModel and self.utils:IsValidModel(data.config.model_path) then
				bot:SetModel(data.config.model_path)
			else
				local fallbackModel = "models/player/urban.mdl"
				if self.config and self.config:get("DEFAULT_MODEL") then
					fallbackModel = self.config:get("DEFAULT_MODEL")
				end
				bot:SetModel(fallbackModel)
			end
		end
