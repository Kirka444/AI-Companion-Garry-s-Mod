...
function Utils:ValidateIP(ip)
    ...
    if string.match(ip, ":%d+$") then return false, "IP не должен содержать порт" end
...
function Utils:CleanText(str, maxLen)
     Utils.Log("[CleanText] ВХОД: str =", tostring(str), "maxLen =", maxLen)
    ...
    if maxLen then
     ...
     Utils.Log("[CleanText] ВЫХОД: str =", tostring(str))
    return str
end

function Utils:CleanForTTS(str)
     Utils.Log("[CleanForTTS] ВХОД: str =", tostring(str), "тип =", type(str))

    if str == nil then
         Utils.Log("[CleanForTTS] ⚠️ str = nil, возвращаем пустую строку")
        return ""
    end

    if type(str) ~= "string" then
         Utils.Log("[CleanForTTS] ⚠️ str не строка, тип:", type(str))
        str = tostring(str) or ""
         Utils.Log("[CleanForTTS] 🔄 Приведён к строке:", str)
    end

    if not str or str == "" then
         Utils.Log("[CleanForTTS] ❌ str пустая, возвращаем пустую строку")
        return ""
    end

    str = tostring(str)
     Utils.Log("[CleanForTTS] После tostring: '" .. str .. "'")
    ...
    str = string.gsub(str, "^%s*(.-)%s*$", "%1")
     ...
     Utils.Log("[CleanForTTS] ВЫХОД: str = '" .. tostring(str) .. "', длина =", #str)

    if str == "" and tostring(str) ~= "" and tostring(str) ~= " " then
         Utils.Log("[CleanForTTS] ⚠️ После очистки строка пуста, возвращаем исходный текст")
        return tostring(str)
    end
...
function Utils:HTTPQueue(params)
     Utils.Log("[HTTPQueue] ========================================")
     Utils.Log("[HTTPQueue] params =", params)
    if params then
         Utils.Log("[HTTPQueue] params.url =", params.url)
         Utils.Log("[HTTPQueue] params.method =", params.method)
    else
         Utils.Log("[HTTPQueue] params = nil!")
    end
     Utils.Log("[HTTPQueue] ========================================")

    if not params or not params.url then
        Utils.LogError("HTTP", "Некорректный запрос: нет URL")
         Utils.Log("[HTTPQueue] ❌ params.url отсутствует!")
        return
    end
