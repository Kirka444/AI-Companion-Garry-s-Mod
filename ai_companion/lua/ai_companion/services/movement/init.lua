...
function Movement:GetLocalized(key, ...)
...
function Movement:init()
	...
	if self.navigation then
		...
		if self.utils then
			self.utils.LogInfo("Movement", self:GetLocalized("log_movement_nav_available"), tostring(navAvailable))
			if not navAvailable then
				self.utils.LogInfo("Movement", self:GetLocalized("log_movement_direct_mode"))
			end
		end
	end
	...
	if self.utils then
		self.utils.LogInfo("Movement", self:GetLocalized("log_movement_init"))
	end
end
...
function Movement:GetBotCache(bot)
	...
	-- Источник правды для стелса — data.config, а не NWBool (защита от рассинхрона)
	local stealthMode = (data and data.config and data.config.stealth_mode) or bot:GetNWBool("AI_StealthMode", false)
	cache = {
		data = data,
		owner = owner,
...
function Movement:ShouldProcessMovement(bot)
	...
	if bot._aiCompanion ~= nil and bot._aiCompCheckT and now - bot._aiCompCheckT < 1.0 then
		return bot._aiCompanion
	end
	...
	bot._aiCompanion = isCompanion
	bot._aiCompCheckT = now
...
function Movement:ProcessMovement(bot, cmd)
	...
	-- Set next heavy update interval based on combat state
	bot._aiNavInterval = isInCombat and 0.033 or 0.066

	-- Sync config NWBools only if they actually changed
	if data and data.config then
		local cfg = data.config
		local c = bot._aiCfg

		-- Принудительная синхронизация: если в конфиге стелс включён, но NWBool сброшен — восстановить
		if cfg.stealth_mode and not bot:GetNWBool("AI_StealthMode", false) then
			bot:SetNWBool("AI_StealthMode", true)
		end

		if not c
			or c.s ~= cfg.stealth_mode
			or c.d ~= cfg.defender_mode
			or c.m ~= cfg.medic_mode
			or c.p ~= cfg.pacifist_mode
			or c.a ~= cfg.aggressive_mode
		then
			bot:SetNWBool("AI_StealthMode", cfg.stealth_mode or false)
			bot:SetNWBool("AI_DefenderMode", cfg.defender_mode or false)
			bot:SetNWBool("AI_MedicMode", cfg.medic_mode or false)
			bot:SetNWBool("AI_PacifistMode", cfg.pacifist_mode or false)
			bot:SetNWBool("AI_AggressiveMode", cfg.aggressive_mode or false)

			bot._aiCfg = {
				s = cfg.stealth_mode or false,
				d = cfg.defender_mode or false,
				m = cfg.medic_mode or false,
				p = cfg.pacifist_mode or false,
				a = cfg.aggressive_mode or false,
			}
		end
	end
	...
	if stealthMode and self:IsValid(owner) and owner:IsPlayer() and owner:Alive() then
		needStealthCrouch = owner:Crouching() or owner:KeyDown(IN_DUCK)
	end
	...
	local targetIsNoclip = self:IsValid(moveTarget)
		and moveTarget:IsPlayer()
		and self.utils_movement:IsTargetNoclip(moveTarget)
	...
	if self:IsValid(owner) and owner:InVehicle() and not bot:InVehicle() and not isInCombat then
		if now > (bot._aiNextVehicleEnterCheck or 0) then
			bot._aiNextVehicleEnterCheck = now + 0.5
						...
						if not self:IsValid(driver) then
							...
							if bot:InVehicle() then
								...
								if bot.ChatPrint then
									bot:ChatPrint(self:GetLocalized("vehicle_enter_driver"))
								end
							end
						end
					end
	...
	if isIdle and not isInCombat then
		...
		local smoothAng = self.utils_movement:LerpAngle(bot:EyeAngles(), lookAng, 12, true)
		cmd:SetViewAngles(smoothAng)
		bot:SetEyeAngles(smoothAng)
		return
	end
	...
	if self:IsValid(moveTarget) then
		...
		if distToTarget > const.teleportDist * const.teleportDist then
			if now > (bot._aiNextTeleportCheck or 0) then
				bot._aiNextTeleportCheck = now + 0.5
				local st = self:GetNavState(bot)
	...
	-- Pass bot data via cache to avoid repeated GetBotData calls
	bot._aiBotData = ctx.data
	-- Track target movement and force repath if target moved significantly
	if self:IsValid(moveTarget) then
		local moveTargetPos = moveTarget:GetPos()
		local lastPos = bot._aiLastTargetPos
		if lastPos then
			local distMoved = moveTargetPos:DistToSqr(lastPos)
			if distMoved > 900 then -- 30 units
				local st = self:GetNavState(bot)
				if st then
					st.repath.force = true
					st.repath.next_force = 0
				end
			end
		end
		bot._aiLastTargetPos = Vector(moveTargetPos)
	end
...
function Movement:CacheCommand(bot, cmd)
	local btns = cmd:GetButtons()

	-- Don't cache jump as held, otherwise bot may start jumping constantly
	btns = bit.band(btns, bit.bnot(IN_JUMP))

	local va = cmd:GetViewAngles()

	bot._aiCmd = {
		f = cmd:GetForwardMove(),
		s = cmd:GetSideMove(),
		u = cmd:GetUpMove(),
		b = btns,
		va = Angle(va.p, va.y, va.r),
		t = CurTime(),
	}
end

function Movement:ApplyCachedCommand(bot, cmd)
	local c = bot._aiCmd

	if not c or CurTime() - c.t > 0.3 then
		cmd:ClearMovement()
		cmd:ClearButtons()
		return
	end

	cmd:SetForwardMove(c.f)
	cmd:SetSideMove(c.s)
	cmd:SetUpMove(c.u)
	cmd:SetButtons(c.b)

	if c.va then
		cmd:SetViewAngles(c.va)
	end
end
...
function Movement:SetupHooks()
	...
	hook.Remove("StartCommand", "gmod.one/ai-companion/movement-start-command")

	local frameNo = -1
	local frameUpdates = 0

	-- How many bots can do heavy navigation update per frame.
	-- If there are many bots, set to 1-2.
	local MAX_NAV_PER_FRAME = 2

	-- Normal navigation update frequency.
	local DEFAULT_NAV_INTERVAL = 0.066

	-- For combat, can be slightly more frequent.
	local COMBAT_NAV_INTERVAL = 0.033

	local disabledCacheT = 0
	local disabledCache = false

	hook.Add("StartCommand", "gmod.one/ai-companion/movement-start-command", function(bot, cmd)
		...
		-- Cache Disabled check to avoid querying state every tick.
		if now - disabledCacheT > 0.25 then
			disabledCache = selfRef.state and selfRef.state:getState("Disabled") or false
			disabledCacheT = now
		end

		if disabledCache then
		...
		-- Vehicle needs more frequent updates, otherwise vehicle control is poor.
		if bot:InVehicle() then
			selfRef:ProcessMovement(bot, cmd)
			selfRef:CacheCommand(bot, cmd)
			bot._aiNextNav = now + 0.016
			return
		end

		-- If heavy update is not needed yet, just apply the last command.
		-- If no path exists, update every tick to build path quickly
		local hasPath = false
		local st = selfRef:GetNavState(bot)
		if st and st.path and #st.path > 0 and st.path_index <= #st.path then
			hasPath = true
		end

		if bot._aiNextNav and now < bot._aiNextNav and hasPath then
			selfRef:ApplyCachedCommand(bot, cmd)
			return
		end

		-- Protection against many bots trying to update in the same frame.
		if frameUpdates >= MAX_NAV_PER_FRAME then
			selfRef:ApplyCachedCommand(bot, cmd)
			bot._aiNextNav = now + 0.03
			return
		end
		...
		bot._aiNextNav = now + (bot._aiNavInterval or DEFAULT_NAV_INTERVAL)
	end)

	hook.Add("PlayerSpawn", "gmod.one/ai-companion/reset-movement-state", function(ply)
		if ply:GetNWBool("IsAICompanion", false) then
			ply._aiSpawnTime = CurTime()

			ply._aiCompanion = nil
			ply._aiCompCheckT = 0
			ply._aiCfg = nil
			ply._aiCmd = nil
			ply._aiNextNav = 0
			ply._aiNavInterval = nil
			ply._aiBotData = nil

			-- Восстанавливаем стелс из конфига вместо жёсткого сброса
			local data = nil
			if selfRef.data then
				data = selfRef.data:GetBotData(ply)
			end
			local stealthMode = data and data.config and data.config.stealth_mode or false
			ply:SetNWBool("AI_StealthMode", stealthMode)
			ply._aiStealthCrouch = stealthMode
	...
	hook.Add("PlayerDeath", "gmod.one/ai-companion/cleanup-movement-state", function(victim)
		if victim:IsBot() and victim:GetNWBool("IsAICompanion", false) then
			...
			victim._aiNextNav = 0
			victim._aiCfg = nil
			victim._aiBotData = nil
		end
	end)

	hook.Add("PlayerDisconnected", "gmod.one/ai-companion/cleanup-movement-cache", function(ply)
